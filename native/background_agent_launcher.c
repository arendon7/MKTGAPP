...
static int fail(const char *message) {
    fprintf(stderr, "BINARIO Marketing background agent failed: %s\n", message);
    return 5;
}
...
int main(void) {
    ...
    int copied = snprintf(contents_dir, sizeof(contents_dir), "%s", macos_dir);
    if (copied < 0 || (size_t)copied >= sizeof(contents_dir) || parent_dir(contents_dir) != 0) {
        return fail("cannot resolve Contents");
    }
    ...
    char keychain_helper[PATH_MAX], bootstrap[PATH_MAX], path_env[PATH_MAX * 3];
    ...
    if (path_join(resources, sizeof(resources), contents_dir, "Resources") ||
        ...
        path_join(bootstrap, sizeof(bootstrap), resources, "background_agent.py")) {
    ...
    if (access(bootstrap, R_OK) != 0) return fail("background bootstrap missing");
    ...
    char *argv[] = {python_bin, "-I", "-B", bootstrap, NULL};
    execve(python_bin, argv, environ);
    return fail("cannot exec embedded background runtime");
}
