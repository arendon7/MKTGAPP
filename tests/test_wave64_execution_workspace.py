...
class Wave64ExecutionWorkspaceTests(unittest.TestCase):
    ...
    def test_ready_creative_flows_to_distribution_without_mutation(self):
        ...
        self.assertEqual(before, self.runtime.creatives.get(self.company["id"], media["id"]))
    ...
    def test_paid_draft_is_linked_without_remote_creation(self):
        ...
        paid = self.runtime.create_company_paid_media(self.company["id"], {
            "campaign_id": campaign["id"], "source_kind": "company_media", "company_media_id": media["id"],
            "campaign_name": "Meta Leads", "campaign_objective": "OUTCOME_LEADS", "special_ad_categories": [],
            "adset_name": "CO Leads", "daily_budget": 10000, "optimization_goal": "LINK_CLICKS",
            "targeting": {"geo_locations": {"countries": ["CO"]}, "age_min": 25, "age_max": 55},
            "creative_name": "Creative W64", "message": "Mensaje de campaña", "link_url": "https://example.com/landing",
            "call_to_action": "LEARN_MORE", "ad_name": "Ad W64",
        })
        self.assertEqual(paid["status"], "DRAFT")
        payload = self.runtime.campaign_execution_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["paid"]["plans"], 1); self.assertEqual(row["paid"]["counts"].get("DRAFT"), 1)
    ...
    def test_workspace_is_company_scoped_and_provider_read_free(self):
        ...
        second = self.runtime.create_campaign(other["id"], {"name": "Solo otra", "objective": "LEADS", "channels": ["facebook_page"]})
    ...
    def test_frontend_is_navigation_only_without_direct_mutations_or_polling(self):
        ...
        for marker in ("Centro de ejecución de campañas", "De plan a distribución", "Solo requieren acción", "Creative Studio", "Calendario", "Pauta", "Resultados", "execution-workspace"):
    ...
    def test_builder_service_workflows_and_release_boundary(self):
        ...
        for audit in ("audit_wave59_local_product_integration.sh", "audit_wave60_daily_workdesk.sh", "audit_wave61_commercial_desk.sh", "audit_wave62_contact_360.sh", "audit_wave63_commercial_pipeline.sh", "audit_wave64_execution_workspace.sh"):
        ...
        self.assertIn("service_wave63_app as base", service); self.assertIn("execution.src='/execution-workspace.js'", service)
