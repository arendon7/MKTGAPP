...
class Wave64ExecutionWorkspaceTests(unittest.TestCase):
    ...
    def _campaign(self, name="Campaña W64", channels=None, status="PLANNING"):
        return self.runtime.create_campaign(self.company["id"], {
            ...
            "objective": "LEADS",
            "status": status,
            "channels": channels if channels is not None else ["facebook_page", "instagram"],
        })
    ...
    def _ready_creative(self, campaign, name="Creative W64"):
        ...
        self.runtime.upsert_company_creative(self.company["id"], media["id"], {
            ...
            "primary_copy": "Mensaje de campaña",
            "public_media_url": "https://cdn.example.com/w64.png",
            "destination_url": "https://example.com/landing",
        })
        return media

    def test_workspace_prioritizes_missing_channels_then_missing_creative(self):
        missing_channels = self._campaign("A sin canales", channels=[])
        missing_creative = self._campaign("B sin creativo")
    ...
    def test_ready_creative_flows_to_distribution_without_mutation(self):
        ...
        media = self._ready_creative(campaign)
        before = self.runtime.creatives.get(self.company["id"], media["id"])
        payload = self.runtime.campaign_execution_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["creative"]["ready"], 1)
        self.assertEqual(row["next_action"]["code"], "PREPARE_DISTRIBUTION")
        self.assertEqual(row["next_action"]["view"], "content")
        after = self.runtime.creatives.get(self.company["id"], media["id"])
        self.assertEqual(before, after)
        self.assertEqual(row["organic"]["publications"], 0)

    def test_prepared_publication_is_reflected_from_canonical_social_store(self):
        campaign = self._campaign()
        media = self._ready_creative(campaign)
        prepared = self.runtime.prepare_creative_publication(self.company["id"], media["id"], {
            "channel": "facebook_page",
            "scheduled_for": "2026-08-25T15:00:00-05:00",
        })
        self.assertEqual(prepared["publication"]["status"], "QUEUED")
        payload = self.runtime.campaign_execution_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["organic"]["counts"].get("QUEUED"), 1)
        self.assertEqual(row["next_action"]["code"], "CALENDAR")
        organic = next(step for step in row["steps"] if step["code"] == "ORGANIC")
        self.assertEqual(organic["state"], "ACTIVE")
    ...
    def test_paid_draft_is_linked_without_remote_creation(self):
        ...
        payload = self.runtime.campaign_execution_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["paid"]["plans"], 1)
        self.assertEqual(row["paid"]["counts"].get("DRAFT"), 1)
        self.assertEqual(row["next_action"]["code"], "REVIEW_PAID")
        self.assertFalse(payload["safety"]["automatic_paid_activation"])
    ...
    def test_workspace_is_company_scoped_and_provider_read_free(self):
        ...
        with patch("binario_marketing.service_wave48_app.MetaGraphClient.from_env", side_effect=AssertionError("provider read forbidden")):
            first_payload = self.runtime.campaign_execution_workspace(self.company["id"])
            second_payload = self.runtime.campaign_execution_workspace(other["id"])
    ...
    def test_http_serves_execution_workspace_and_wave64_bootstrap(self):
        ...
        try:
            ...
            with urlopen(base + "/commercial-pipeline.js", timeout=5) as response:
                pipeline_ui = response.read().decode("utf-8")
            self.assertIn("execution-workspace.js", pipeline_ui)
            self.assertIn("data-execution-workspace-wave64", pipeline_ui)
            ...
            with urlopen(base + f"/api/companies/{self.company['id']}/execution-workspace", timeout=5) as response:
    ...
    def test_frontend_is_navigation_only_without_direct_mutations_or_polling(self):
        ...
        for marker in (
            "Centro de ejecución de campañas",
            "De plan a distribución",
        ...
        self.assertIn("opsShowView(view)", ui)
        self.assertIn("opsShowLegacy()", ui)
    ...
    def test_builder_service_workflows_and_release_boundary(self):
        ...
        service = (ROOT / "src" / "binario_marketing" / "service_wave64_app.py").read_text(encoding="utf-8")
        ...
        for wave in (59, 60, 61, 62, 63, 64):
            self.assertIn(f"CURRENT ARM64 ITERATION BUILD PASS: Wave {wave}", builder)
        self.assertIn("service_wave63_app as base", service)
        self.assertIn("execution.src='/execution-workspace.js'", service)
