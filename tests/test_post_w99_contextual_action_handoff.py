...
class PostW99ContextualActionHandoffTests(unittest.TestCase):
    ...
    def test_runtime_preserves_evidence_observability_and_prior_chain(self):
    ...
    def test_browser_bootstrap_is_appended_after_evidence_observability(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime = AppRuntime.create(ROOT, Path(tmp) / "data")
            ...
            try:
                ...
                self.assertIn("/contextual-action-handoff.js", evidence)
    ...
    def test_handoff_layer_owns_no_business_transport_or_synthetic_execution(self):
        ...
        self.assertIn("service_post_w99_evidence_observability_integrated_app as base", service)

    def test_dev_entrypoint_and_docs_preserve_release_boundary(self):
        ...
        self.assertIn("Evidence Observability → Contextual Action Handoff", dev_docs)
