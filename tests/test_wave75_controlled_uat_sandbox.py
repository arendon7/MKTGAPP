...
class Wave75ControlledUATSandboxTests(unittest.TestCase):
    ...
    def test_create_is_idempotent_and_reset_only_deactivates_recorded_sandbox(self):
        ...
        first = self.runtime.create_uat_sandbox({}); same = self.runtime.create_uat_sandbox({})
        self.assertEqual(first["company"]["id"], same["company"]["id"]); self.assertEqual(first["generation"], same["generation"])
        reset = self.runtime.reset_uat_sandbox({"confirm": True})
        self.assertNotEqual(first["company"]["id"], reset["company"]["id"]); self.assertEqual(reset["generation"], first["generation"] + 1)
        self.assertFalse(self.runtime.companies.get(first["company"]["id"]).active); self.assertTrue(self.runtime.companies.get(reset["company"]["id"]).active); self.assertTrue(self.runtime.companies.get(real["id"]).active)
        self.assertTrue(self.runtime.uat_sandbox.is_sandbox(first["company"]["id"])); self.assertTrue(self.runtime.uat_sandbox.is_sandbox(reset["company"]["id"]))
        with self.assertRaises(ValueError): self.runtime.reset_uat_sandbox({"confirm": False})
    ...
    def test_http_routes_are_explicit_and_ui_never_autocreates_or_polls(self):
        server = create_server(self.runtime, "127.0.0.1", 0); thread = threading.Thread(target=server.serve_forever, daemon=True); thread.start(); base = f"http://127.0.0.1:{server.server_address[1]}"
        try:
            with urlopen(base + "/api/uat-sandbox", timeout=10) as response: initial = json.loads(response.read()); self.assertFalse(initial["exists"])
            request = Request(base + "/api/uat-sandbox", data=b"{}", headers={"Content-Type":"application/json"}, method="POST")
            with urlopen(request, timeout=10) as response: created = json.loads(response.read()); self.assertEqual(response.status, 201)
            request = Request(base + "/api/uat-sandbox/reset", data=b'{"confirm":true}', headers={"Content-Type":"application/json"}, method="POST")
            with urlopen(request, timeout=10) as response: reset = json.loads(response.read()); self.assertEqual(response.status, 201)
            self.assertNotEqual(created["company"]["id"], reset["company"]["id"])
            with urlopen(base + "/interaction-audit.js", timeout=10) as response: audit = response.read().decode()
            self.assertIn("/uat-sandbox.js", audit)
            with urlopen(base + "/uat-sandbox.js", timeout=10) as response: ui = response.read().decode()
            self.assertIn("SINTÉTICO · NO RELEASE", ui); self.assertIn("window.confirm", ui); self.assertNotIn("setInterval", ui); self.assertNotIn("/api/meta/", ui); self.assertNotIn("/ai/generate", ui); self.assertNotIn("publish", ui.lower())
        finally:
    ...
    def test_release_contract_workflows_and_builder_stay_fail_closed(self):
        ...
        for wave in range(66, 75): self.assertIn(f"Wave {wave}", builder)
