...
class Wave65ResultsIntelligenceTests(unittest.TestCase):
    ...
    def _snapshot(self, campaign, *, reach=100, interactions=10, clicks=0):
        ...
        if clicks:
            paid.append({
                ...
                "currency": "COP",
                "metrics": {"impressions": 1000, "reach": 800, "clicks": clicks, "spend": 25000},
            })
        return self.runtime.learning.create_snapshot(self.company["id"], {
            "date_preset": "last_7d",
            "social": {
                "coverage": {"eligible": 1, "requested": 1, "measured": 1, "errors": 0},
                "totals": {"reach": reach, "total_interactions": interactions},
                "observations": [{
                    "campaign_id": campaign["id"],
                    "creative_media_id": None,
                    "metrics": {"reach": reach, "total_interactions": interactions},
                }],
            },
            "paid_media": {
                "coverage": {"eligible": len(paid), "requested": len(paid), "measured": len(paid), "errors": 0},
                "currencies": ["COP"] if paid else [],
                "spend_aggregated": True,
                "totals": {"clicks": clicks, "spend": 25000} if paid else {},
                "observations": paid,
            },
            "crm": {},
            "coverage": {"social": {"measured": 1}, "paid_media": {"measured": len(paid)}},
        })

    def test_observed_evidence_requires_human_decision_before_ai(self):
        campaign = self._campaign()
        snapshot = self._snapshot(campaign, clicks=20)
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        ...
        self.assertEqual(row["evidence"]["metrics"]["paid_ctr"], 2.0)
        self.assertEqual(row["next_action"]["code"], "RECORD_DECISION")
        self.assertTrue(row["requires_attention"])
        self.assertEqual(payload["latest_snapshot"]["id"], snapshot.id)
        self.assertFalse(payload["safety"]["ai_generation_performed"])
    ...
    def test_recorded_decision_keeps_ai_optional_and_non_executing(self):
        ...
        self.runtime.record_learning_decision(self.company["id"], {
            ...
            "action": "ITERATE",
            "rationale": "La evidencia observada justifica probar una variante.",
            "snapshot_id": snapshot.id,
        })
        self.runtime.ai_settings.update(self.company["id"], {"provider": "ollama", "model": "llama3.2"})
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["decision"]["action"], "ITERATE")
        self.assertEqual(row["next_action"]["code"], "OPTIONAL_AI")
        self.assertFalse(row["requires_attention"])
        self.assertTrue(payload["ai"]["configured"])
        self.assertFalse(payload["ai"]["marketing_execution_authority"])
        self.assertTrue(payload["ai"]["generation_requires_explicit_user_action"])
    ...
    def test_partial_attribution_is_separate_from_observed_metrics(self):
        ...
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["evidence"]["level"], "ATTRIBUTED_WON")
        self.assertFalse(row["evidence"]["observed"])
        self.assertEqual(row["attribution"]["attributed_opportunities"], 1)
        self.assertEqual(row["attribution"]["attributed_won"], 1)
        self.assertEqual(row["attribution"]["value_by_currency"]["COP"]["won_value"], 2500000)
        self.assertEqual(row["attribution"]["model"], "LAST_CAPTURED_TOUCH")
        self.assertFalse(payload["attribution_coverage"]["full_funnel_coverage_assumed"])
    ...
    def test_latest_ai_is_compact_history_not_source_of_truth(self):
        ...
        self.runtime.record_learning_decision(self.company["id"], {
            ...
            "entity_id": campaign["id"],
            "action": "HOLD",
            "rationale": "Esperar más evidencia.",
            "snapshot_id": snapshot.id,
        })
        self.runtime.ai_settings.update(self.company["id"], {"provider": "ollama", "model": "llama3.2"})
        context = {"schema": "test", "privacy": {"contact_pii_included": False}}
        ...
        session = self.runtime.ai_sessions.create(
            ...
            campaign_id=campaign["id"],
            creative_media_id=None,
            instruction="Analiza evidencia",
            context_sha256=digest,
            context=context,
            output={
                "summary": "Mantener y observar.",
                "diagnosis": ["La muestra todavía es limitada."],
                "recommendations": [{
                    "title": "Esperar más señal",
                    "why": "La cobertura aún es corta.",
                    "priority": "MEDIUM",
                    "area": "CAMPAIGN",
                    "next_step": "Capturar otro snapshot.",
                }],
                "creative_variants": [],
                "campaign_brief": {},
            },
        ...
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["latest_ai"]["id"], session.id)
        self.assertEqual(row["latest_ai"]["summary"], "Mantener y observar.")
        self.assertEqual(row["latest_ai"]["recommendation_count"], 1)
        self.assertNotIn("context", row["latest_ai"])
        self.assertEqual(payload["summary"]["with_ai_analysis"], 1)
        self.assertFalse(payload["safety"]["automatic_recommendation_execution"])

    def test_workspace_is_company_scoped_and_provider_read_free(self):
        first = self._campaign("Greenatics only")
        other = self.runtime.create_company({"name": "Otra empresa"})
        second = self.runtime.create_campaign(other["id"], {
            "name": "Otra only", "objective": "LEADS", "status": "IN_PROGRESS", "channels": ["instagram"]
        })
        with patch.object(self.runtime, "social_analytics_meta", side_effect=AssertionError("provider read forbidden")), \
            ...
            first_payload = self.runtime.results_intelligence_workspace(self.company["id"])
            second_payload = self.runtime.results_intelligence_workspace(other["id"])
        self.assertEqual({row["campaign"]["id"] for row in first_payload["campaigns"]}, {first["id"]})
        self.assertEqual({row["campaign"]["id"] for row in second_payload["campaigns"]}, {second["id"]})
        self.assertFalse(first_payload["safety"]["provider_read_performed"])
        self.assertFalse(first_payload["safety"]["provider_mutation_performed"])
        self.assertFalse(first_payload["safety"]["background_polling"])
        self.assertFalse(first_payload["safety"]["cloud_required"])
    ...
    def test_http_serves_results_workspace_and_wave65_bootstrap(self):
        ...
        try:
            ...
            with urlopen(base + "/execution-workspace.js", timeout=5) as response:
                execution_ui = response.read().decode("utf-8")
            self.assertIn("results-intelligence.js", execution_ui)
            self.assertIn("data-results-intelligence-wave65", execution_ui)
    ...
    def test_frontend_only_generates_ai_explicitly_and_never_executes_marketing(self):
        ...
        for marker in (
            "Resultados & IA",
            "Evidencia determinística primero",
            "LAST_CAPTURED_TOUCH",
            "Solo requieren atención",
            "Analizar con IA",
            "/ai/generate",
            "La IA no publicará, activará pauta ni ejecutará decisiones",
        ):
            self.assertIn(marker, ui)
        self.assertEqual(ui.count("method:'POST'"), 1)
        for forbidden in ("method:'PATCH'", "method:'PUT'", "method:'DELETE'", "setInterval", "sendBeacon", "fetch('https://"):
            self.assertNotIn(forbidden, ui)
        self.assertIn("confirm(`Se enviará contexto marketing sanitizado", ui)
        self.assertIn("task:'CAMPAIGN'", ui)
        self.assertNotIn("/learning/refresh", ui)
    ...
    def test_builder_service_workflows_and_release_boundary(self):
        ...
        service = (ROOT / "src" / "binario_marketing" / "service_wave65_app.py").read_text(encoding="utf-8")
        ...
        for wave in (59, 60, 61, 62, 63, 64, 65):
            self.assertIn(f"CURRENT ARM64 ITERATION BUILD PASS: Wave {wave}", builder)
        self.assertIn("service_wave64_app as base", service)
        self.assertIn("intelligence.src='/results-intelligence.js'", service)
