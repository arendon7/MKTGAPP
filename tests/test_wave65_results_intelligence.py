...
class Wave65ResultsIntelligenceTests(unittest.TestCase):
    ...
    def test_observed_evidence_requires_human_decision_before_ai(self):
        campaign = self._campaign(); snapshot = self._snapshot(campaign, clicks=20)
        ...
        self.assertEqual(row["evidence"]["level"], "OBSERVED"); self.assertEqual(row["evidence"]["metrics"]["paid_ctr"], 2.0)
        self.assertEqual(row["next_action"]["code"], "RECORD_DECISION"); self.assertTrue(row["requires_attention"])
        self.assertEqual(payload["latest_snapshot"]["id"], snapshot.id); self.assertFalse(payload["safety"]["ai_generation_performed"])
    ...
    def test_recorded_decision_keeps_ai_optional_and_non_executing(self):
        ...
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["decision"]["action"], "ITERATE"); self.assertEqual(row["next_action"]["code"], "OPTIONAL_AI")
        self.assertFalse(row["requires_attention"]); self.assertTrue(payload["ai"]["configured"])
        self.assertFalse(payload["ai"]["marketing_execution_authority"]); self.assertTrue(payload["ai"]["generation_requires_explicit_user_action"])
    ...
    def test_partial_attribution_is_separate_from_observed_metrics(self):
        ...
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["evidence"]["level"], "ATTRIBUTED_WON"); self.assertFalse(row["evidence"]["observed"])
        self.assertEqual(row["attribution"]["attributed_opportunities"], 1); self.assertEqual(row["attribution"]["attributed_won"], 1)
        self.assertEqual(row["attribution"]["value_by_currency"]["COP"]["won_value"], 2500000); self.assertEqual(row["attribution"]["model"], "LAST_CAPTURED_TOUCH")
    ...
    def test_latest_ai_is_compact_history_not_source_of_truth(self):
        ...
        payload = self.runtime.results_intelligence_workspace(self.company["id"])
        row = next(item for item in payload["campaigns"] if item["campaign"]["id"] == campaign["id"])
        self.assertEqual(row["latest_ai"]["id"], session.id); self.assertEqual(row["latest_ai"]["summary"], "Mantener y observar."); self.assertEqual(row["latest_ai"]["recommendation_count"], 1)
        self.assertNotIn("context", row["latest_ai"]); self.assertEqual(payload["summary"]["with_ai_analysis"], 1); self.assertFalse(payload["safety"]["automatic_recommendation_execution"])

    def test_workspace_is_company_scoped_and_provider_read_free(self):
        first = self._campaign("Greenatics only"); other = self.runtime.create_company({"name": "Otra empresa"})
        second = self.runtime.create_campaign(other["id"], {"name": "Otra only", "objective": "LEADS", "status": "IN_PROGRESS", "channels": ["instagram"]})
        ...
        self.assertFalse(first_payload["safety"]["provider_read_performed"]); self.assertFalse(first_payload["safety"]["provider_mutation_performed"]); self.assertFalse(first_payload["safety"]["background_polling"]); self.assertFalse(first_payload["safety"]["cloud_required"])
    ...
    def test_frontend_only_generates_ai_explicitly_and_never_executes_marketing(self):
        ...
        for marker in ("Resultados & IA", "Evidencia determinística primero", "LAST_CAPTURED_TOUCH", "Solo requieren atención", "Analizar con IA", "/ai/generate", "La IA no publicará, activará pauta ni ejecutará decisiones"):
    ...
    def test_builder_service_workflows_and_release_boundary(self):
        ...
        for audit in ("audit_wave59_local_product_integration.sh", "audit_wave60_daily_workdesk.sh", "audit_wave61_commercial_desk.sh", "audit_wave62_contact_360.sh", "audit_wave63_commercial_pipeline.sh", "audit_wave64_execution_workspace.sh", "audit_wave65_results_intelligence.sh"):
        ...
        self.assertIn("service_wave64_app as base", service); self.assertIn("intelligence.src='/results-intelligence.js'", service)
        self.assertNotIn("def do_POST", service); self.assertNotIn("def do_PATCH", service); self.assertNotIn("def do_DELETE", service); self.assertIn('host: str = "127.0.0.1"', service)
