...
class Wave76SandboxFunctionalJourneyTests(unittest.TestCase):
    ...
    def test_observer_detects_only_operator_state_changes(self):
        sandbox = self.runtime.create_uat_sandbox({}); cid = sandbox["company"]["id"]; entities = sandbox["entities"]; contact_id = entities["contact_id"]
        self.runtime.convert_lead(cid, entities["matched_lead_id"], {"action": "LINK_CONTACT", "contact_id": contact_id})
        journey = self.runtime.uat_sandbox_journey(); self.assertEqual(journey["summary"]["core_verified"], 3); self.assertEqual(journey["next_checkpoint"]["code"], "NEW_LEAD_HANDOFF")
        self.runtime.convert_lead(cid, entities["new_lead_id"], {"action": "CREATE_CONTACT"})
        self.runtime.crm.update_opportunity(cid, entities["opportunity_id"], {"stage": "INTERESTED"}); self.runtime.crm.complete_activity(cid, entities["activity_id"])
        final = self.runtime.uat_sandbox_journey(); self.assertTrue(final["summary"]["core_complete"], final); self.assertEqual(final["summary"]["core_verified"], 6)
        by_code = {row["code"]: row for row in final["checkpoints"]}
        for code in ("EXACT_MATCH_HANDOFF", "NEW_LEAD_HANDOFF", "PIPELINE_STAGE_SAVE", "FOLLOWUP_INTERACTION"):
    ...
    def test_real_company_never_becomes_the_observed_sandbox(self):
        real = self.runtime.create_company({"name": "Empresa real aislada"}); sandbox = self.runtime.create_uat_sandbox({}); journey = self.runtime.uat_sandbox_journey()
        self.assertNotEqual(real["id"], sandbox["company"]["id"]); self.assertEqual(journey["sandbox"]["company"]["id"], sandbox["company"]["id"]); self.assertTrue(self.runtime.companies.get(real["id"]).active)
        self.runtime.reset_uat_sandbox({"confirm": True}); self.assertTrue(self.runtime.companies.get(real["id"]).active)

    def test_http_projection_and_browser_layer_are_read_only(self):
        self.runtime.create_uat_sandbox({}); server = create_server(self.runtime, "127.0.0.1", 0); thread = threading.Thread(target=server.serve_forever, daemon=True); thread.start(); base = f"http://127.0.0.1:{server.server_address[1]}"
        try:
            with urlopen(base + "/api/uat-sandbox/journey", timeout=10) as response: payload = json.loads(response.read())
            self.assertEqual(payload["schema"], "binario.marketing.uat-sandbox-journey.v1"); self.assertTrue(payload["safety"]["read_only_projection"])
            with urlopen(base + "/uat-sandbox.js", timeout=10) as response: parent = response.read().decode()
            self.assertIn("/uat-functional-journey.js", parent)
            with urlopen(base + "/uat-functional-journey.js", timeout=10) as response: ui = response.read().decode()
            self.assertIn("FUNCTIONAL JOURNEY VALIDATOR · W76", ui); self.assertIn("Verificar cambios", ui); self.assertNotIn("setInterval", ui); self.assertNotIn("/api/meta/", ui); self.assertNotIn("/ai/generate", ui); self.assertNotIn("method:'POST'", ui); self.assertNotIn("method:'PATCH'", ui); self.assertNotIn("method:'DELETE'", ui)
        finally:
    ...
    def test_release_workflow_and_current_builder_boundaries(self):
        ...
        self.assertNotIn("RELEASE_READY = True", service); self.assertNotIn("social_inbox(", service); self.assertNotIn("publish_company_publication_now", service)
        builder = (ROOT / "scripts/build_full_mac_current.sh").read_text(encoding="utf-8")
        for wave in range(66, 76): self.assertIn(f"Wave {wave}", builder)
