...
class Wave63CommercialPipelineTests(unittest.TestCase):
    ...
    def test_pipeline_prioritizes_overdue_then_missing_followup(self):
        ...
        overdue = self.runtime.create_opportunity(self.company["id"], {
            "contact_id": contact["id"], "title": "A vencida", "stage": "NEW", "currency": "COP"
        })
        missing = self.runtime.create_opportunity(self.company["id"], {
            "contact_id": contact["id"], "title": "B sin acción", "stage": "NEW", "currency": "COP"
        })
    ...
    def test_http_serves_pipeline_bootstrap_and_inherited_explicit_patch(self):
        ...
        try:
            ...
            with urlopen(base + "/contact-360.js", timeout=5) as response:
                contact_ui = response.read().decode("utf-8")
