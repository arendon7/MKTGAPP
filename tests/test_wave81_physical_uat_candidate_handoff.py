...
class Wave81PhysicalUATCandidateHandoffTests(unittest.TestCase):
    ...
    def test_writer_creates_exact_fail_closed_candidate_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            proc = subprocess.run([sys.executable, str(ROOT / "scripts/write_physical_uat_candidate.py"), "--app", str(app)], capture_output=True, text=True, check=False)
            ...
            verify = subprocess.run([sys.executable, str(ROOT / "scripts/write_physical_uat_candidate.py"), "--app", str(app), "--verify"], capture_output=True, text=True, check=False)
            self.assertEqual(verify.returncode, 0, verify.stdout + verify.stderr)

    def test_manifest_verification_detects_source_drift(self):
        with tempfile.TemporaryDirectory() as tmp:
            app = self._fake_app(Path(tmp))
            subprocess.run([sys.executable, str(ROOT / "scripts/write_physical_uat_candidate.py"), "--app", str(app)], check=True, capture_output=True, text=True)
    ...
    def test_release_uat_collection_requires_physical_arm64_non_ci_candidate(self):
        ...
        self.assertIn('provenance.get("architecture")=="arm64"', compact)
        self.assertIn('host_system=="Darwin"', compact)
        self.assertIn('host_machine=="arm64"', compact)
        self.assertIn('notis_ci', compact)
        ...
        self.assertIn('trusted_build_origin', source)
