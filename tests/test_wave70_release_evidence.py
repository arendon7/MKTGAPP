...
class Wave70ReleaseEvidenceTests(unittest.TestCase):
    ...
    def _passed_session(self, build=None, machine=None):
        ...
        with patch("binario_marketing.physical_uat_store.machine_snapshot", return_value=machine):
            row = self.runtime.physical_uat.create(self.company["id"], scenarios=list(readiness["manual_scenarios"]), build=build, operator="UAT")
        for scenario in row["scenarios"]:
            status = "PASS" if scenario["required"] else "SKIPPED"
            row = self.runtime.physical_uat.update_scenario(self.company["id"], row["id"], scenario["id"], {"status": status, "note": "verificado"})
        return self.runtime.physical_uat.finish(self.company["id"], row["id"], readiness=readiness)
    ...
    def test_exact_build_physical_pass_satisfies_only_uat_blocker(self):
        ...
        self.assertNotIn("development_version", blockers)
        self.assertNotIn("release_flag_false", blockers)
        self.assertNotIn("release_tag_missing", blockers)
    ...
    def test_tampered_evidence_digest_is_rejected(self):
        ...
        row = json.loads(path.read_text(encoding="utf-8")); row["operator"] = "tampered"; path.write_text(json.dumps(row), encoding="utf-8")
        with patch.object(self.runtime, "_build_provenance", return_value=dict(self.build)):
            payload = self.runtime.release_evidence(self.company["id"])
        latest = payload["physical_uat"]["latest_validation"]
        self.assertFalse(latest["digest_valid"]); self.assertIn("evidence_digest_mismatch", latest["rejection_reasons"])
        self.assertIn("physical_uat_missing", payload["release_readiness"]["blocker_codes"])
    ...
    def test_ci_session_never_satisfies_physical_gate(self):
        ...
        session = self._passed_session(machine=ci_machine); self.assertFalse(session["physical_uat_complete"])
        with patch.object(self.runtime, "_build_provenance", return_value=dict(self.build)):
            payload = self.runtime.release_evidence(self.company["id"])
        latest = payload["physical_uat"]["latest_validation"]
        self.assertFalse(latest["machine_eligible"]); self.assertIn("machine_ineligible", latest["rejection_reasons"])
        self.assertIn("physical_uat_missing", payload["release_readiness"]["blocker_codes"])
    ...
    def test_http_projection_is_get_only_and_browser_has_no_release_authority(self):
        ...
        try:
            ...
            with urlopen(base + "/physical-uat-preflight.js", timeout=5) as response: chained = response.read().decode("utf-8")
            self.assertIn("release-evidence.js", chained); self.assertIn("data-release-evidence-wave70", chained)
            with urlopen(base + "/release-evidence.js", timeout=5) as response: ui = response.read().decode("utf-8")
            ...
            with urlopen(base + f"/api/companies/{self.company['id']}/release-evidence", timeout=5) as response: payload = json.loads(response.read().decode("utf-8"))
            self.assertTrue(payload["safety"]["read_only"]); self.assertFalse(payload["safety"]["release_state_mutation_performed"])
        finally:
    ...
    def test_release_contract_and_workflow_count_remain_non_authoritative(self):
        ...
        self.assertNotIn("RELEASE_READY = True", service); self.assertIn("physical_uat_can_remove_only_uat_blocker", service)
