...
from binario_marketing import service_post_w99_campaign_coordinate_recovery_guidance_app as recovery_base
...
class PostW99CampaignExecutionOwnerCardinalityHardeningTests(unittest.TestCase):
    ...
    def test_terminal_inherits_current_recovery_guidance_chain(self):
        self.assertTrue(issubclass(AppRuntime, recovery_base.AppRuntime))
    ...
    def test_finish_creative_multiple_incomplete_is_ambiguous_even_when_w64_selected_one(self):
        resolution = _harden_media_resolution(
            ...
            inherited_resolution={"state": "EXACT_TARGET", "target_id": "draft-a"},
        )
    ...
    def test_unique_candidate_must_match_w64_and_ineligible_rows_do_not_contaminate(self):
        exact = _harden_media_resolution(
        ...
        self.assertEqual((exact["state"], exact["target_id"]), ("EXACT_TARGET", "draft-only"))
        mismatch = _harden_media_resolution(
            next_action={"code": "FINISH_CREATIVE", "media_id": "stale"},
            linked_creatives=[creative("actual", "DRAFT"), creative("ready-other", "READY")],
            inherited_resolution={},
        )
        self.assertEqual(mismatch["state"], "NO_TARGET")
        self.assertIsNone(mismatch["target_id"])
    ...
    def test_non_media_codes_preserve_parent_resolution(self):
        ...
        resolution = _harden_media_resolution(
            next_action={"code": "COORDINATE"}, linked_creatives=[], inherited_resolution=inherited
        )
        self.assertEqual(resolution, inherited)
        self.assertIsNot(resolution, inherited)

    def test_runtime_hardens_media_without_losing_coordinate_recovery(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime = AppRuntime.create(ROOT, Path(tmp) / "data")
            try:
                company = runtime.create_company({"name": "Cardinality Recovery Co"})
                campaign = runtime.campaigns.create(company["id"], {
                    "name": "Dos borradores", "objective": "LEADS", "status": "IN_PROGRESS",
    ...
    def test_browser_hardening_preserves_canonical_submit_invariants_and_zero_transport(self):
        source = (ROOT / "web" / "campaign-execution-owner-cardinality-hardening.js").read_text(encoding="utf-8")
        for required in (
            "controlHandoffSingleGroup", "Guardar ficha creativa", "Guardar cambios",
            "W49_FINISH_CREATIVE_CANONICAL_SUBMIT", "W35_DEFINE_CHANNELS_CANONICAL_SUBMIT",
        ):
    ...
    def test_http_bootstrap_is_after_recovery_guidance_and_parent_endpoint_survives(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime = AppRuntime.create(ROOT, Path(tmp) / "data")
            company = runtime.create_company({"name": "Hardening HTTP"})
            campaign = runtime.campaigns.create(company["id"], {
                "name": "Exact", "objective": "LEADS", "status": "IN_PROGRESS", "channels": []
            })
            server = create_server(runtime, "127.0.0.1", 0)
            thread = threading.Thread(target=server.serve_forever, daemon=True); thread.start()
            root = f"http://127.0.0.1:{server.server_address[1]}"
            try:
                parent = urlopen(root + "/campaign-coordinate-recovery-guidance.js", timeout=5).read().decode("utf-8")
                child = urlopen(root + "/campaign-execution-owner-cardinality-hardening.js", timeout=5).read().decode("utf-8")
                ...
                self.assertIn("Guardar ficha creativa", child)
    ...
    def test_service_and_docs_preserve_frozen_release_boundary(self):
        ...
        self.assertIn("service_post_w99_campaign_coordinate_recovery_guidance_app", service)
