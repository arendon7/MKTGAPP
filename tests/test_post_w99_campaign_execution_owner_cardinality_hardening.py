...
from binario_marketing import service_post_w99_campaign_coordinate_state_decomposition_app as coordinate_base
...
class PostW99CampaignExecutionOwnerCardinalityHardeningTests(unittest.TestCase):
    ...
    def test_terminal_inherits_current_coordinate_decomposition_chain(self):
        self.assertTrue(issubclass(AppRuntime, coordinate_base.AppRuntime))
    ...
    def test_unique_semantic_candidate_is_exact_and_ineligible_rows_do_not_contaminate(self):
    ...
    def test_runtime_turns_positional_finish_creative_into_ambiguity_without_losing_coordinate_layer(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                rows = [
                    row for row in runtime.action_center(company["id"])["queue"]
                    if row.get("kind") == "finish_creative"
                ]
    ...
    def test_http_bootstrap_is_static_only_and_preserves_current_parent_chain(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime = AppRuntime.create(ROOT, Path(tmp) / "data")
            company = runtime.create_company({"name": "Hardening HTTP"})
            campaign = runtime.campaigns.create(
                company["id"],
            ...
            try:
                parent = urlopen(root + "/campaign-creative-creation-intent-handoff.js", timeout=5).read().decode("utf-8")
                hardening = urlopen(root + "/campaign-execution-owner-cardinality-hardening.js", timeout=5).read().decode("utf-8")
                context = json.loads(
                    urlopen(
                        root + f"/api/companies/{company['id']}/campaigns/{campaign.id}/execution-owner-context",
                        timeout=5,
                    ).read().decode("utf-8")
                )
    ...
    def test_service_and_docs_preserve_frozen_release_boundary(self):
        service = (
            ROOT / "src" / "binario_marketing"
            / "service_post_w99_campaign_execution_owner_cardinality_hardening_app.py"
        ).read_text(encoding="utf-8")
        entrypoint = (
            ROOT / "src" / "binario_marketing" / "service_post_w99_dev_app.py"
        ).read_text(encoding="utf-8")
        doc = (
            ROOT / "docs" / "POST_W99_CAMPAIGN_EXECUTION_OWNER_CARDINALITY_HARDENING.md"
        ).read_text(encoding="utf-8")
        self.assertIn("service_post_w99_campaign_coordinate_state_decomposition_app", service)
