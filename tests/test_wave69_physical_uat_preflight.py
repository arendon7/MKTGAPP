...
class Wave69PhysicalUATPreflightTests(unittest.TestCase):
    ...
    def test_release_contract_remains_fail_closed(self):
        ...
        report=source_release_readiness(); self.assertTrue(report["source_ready"]); self.assertEqual(report["stage"],"SOURCE_CONTRACT_READY"); self.assertFalse(report["operational_inputs_complete"]); self.assertFalse(report["production_ready"])
