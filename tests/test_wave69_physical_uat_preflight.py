...
class Wave69PhysicalUATPreflightTests(unittest.TestCase):
    ...
    def _eligible_machine():
        return {"system": "Darwin", "macos_version": "15.7.7", "machine": "arm64", "is_ci": False, "physical_gate_eligible": True}
    ...
    def _fake_packaged_runtime(self, *, trusted=True):
        ...
        executables = [runtime / "python/bin/python3", runtime / "media/bin/ffmpeg", runtime / "media/bin/ffprobe", runtime / "transcription/bin/whisper-cli"]
        ...
        (resources / "PHYSICAL_UAT_CANDIDATE.json").write_text(json.dumps({
            "schema":"binario.marketing.physical-uat-candidate.v1",
            "role":"PHYSICAL_UAT_CANDIDATE_ONLY" if trusted else "VALIDATION_BUILD_ONLY",
            "git_sha":"a"*40,"architecture":"arm64","product_version":"0.9.0.dev1","runtime_wave":76,"certification_guard_wave":83,
            "candidate_source_sha256":"b"*64,
            "build_origin":{"event":"push" if trusted else "pull_request","ref":"refs/heads/main" if trusted else "refs/pull/90/merge","trusted_for_physical_uat":trusted},
    ...
    def test_source_checkout_is_not_misreported_as_physical_bundle_ready(self):
        ...
        self.assertIn("trusted-build-candidate", payload["blockers"])
    ...
    def test_packaged_arm64_runtime_can_pass_preflight_without_completing_uat(self):
        ...
        self.assertTrue(payload["candidate"]["trusted_for_physical_uat"])
    ...
    def test_validation_pr_bundle_cannot_start_physical_uat(self):
        self._fake_packaged_runtime(trusted=False)
        machine = self._eligible_machine()
        with patch("binario_marketing.service_wave69_app.machine_snapshot", return_value=machine):
            payload = self.runtime.physical_uat_preflight(self.company["id"])
            self.assertFalse(payload["ready_to_begin_physical_uat"])
            self.assertIn("trusted-build-candidate", payload["blockers"])
            self.assertEqual(payload["candidate"]["role"], "VALIDATION_BUILD_ONLY")
            with self.assertRaisesRegex(ValueError, "trusted-build-candidate"):
    ...
    def test_active_session_changes_next_action_but_preflight_records_no_result(self):
        ...
        self.assertEqual(payload["active_session_id"], session["id"]); self.assertEqual(payload["next_action"]["code"], "CONTINUE_SESSION")
    ...
    def test_builder_keeps_release_fail_closed(self):
