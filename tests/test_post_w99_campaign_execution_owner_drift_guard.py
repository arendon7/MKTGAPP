...
def payload(row):
    return {
        ...
        "contracts": {"setup_shadow_deduplication_fail_closed": True},
    }
...
class OwnerDriftProjectionTests(unittest.TestCase):
    ...
    def test_annotation_preserves_priority_action_and_resolution(self):
        row = drift_row("REVIEW_PAID", "pauta")
        original = copy.deepcopy(row)
        result = annotate_campaign_execution_owner_drift(payload(row))
        current = result["queue"][0]
        self.assertEqual(current["rank"], original["rank"])
        self.assertEqual(current["urgency"], original["urgency"])
        self.assertEqual(current["blocking"], original["blocking"])
        self.assertEqual(current["action"], original["action"])
        self.assertEqual(current["owner_resolution"], original["owner_resolution"])
...
class OwnerDriftCompositionTests(unittest.TestCase):
    def test_terminal_inherits_media_selection(self):
    ...
    def test_service_bootstraps_after_media_selection_and_is_get_only(self):
        source = (
            ROOT / "src" / "binario_marketing" / "service_post_w99_campaign_execution_owner_drift_guard_app.py"
        ).read_text(encoding="utf-8")
        self.assertIn("service_post_w99_campaign_media_candidate_selection_handoff_app", source)
        self.assertIn('path == "/campaign-media-candidate-selection-handoff.js"', source)
