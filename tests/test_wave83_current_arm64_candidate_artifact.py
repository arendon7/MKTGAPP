...
def _load_packager():
    path=ROOT/"scripts/package_current_arm64_candidate.py"; spec=importlib.util.spec_from_file_location("wave83_packager",path); assert spec and spec.loader
...
class Wave83CurrentArm64CandidateArtifactTests(unittest.TestCase):
    ...
    def test_external_delivery_metadata_keeps_exact_candidate_truth_with_later_guard(self):
    ...
    def _manifest(module, git_sha: str, *, trusted: bool) -> dict:
        return {
            ...
            "git_sha":git_sha,"architecture":"arm64","product":"BINARIO Marketing IA","product_version":"0.9.0.dev1","runtime_wave":76,"certification_guard_wave":84,
            "candidate_source_sha256":"e"*64,
            "build_origin":{"event":"push" if trusted else "pull_request","ref":"refs/heads/main" if trusted else "refs/pull/92/merge","trusted_for_physical_uat":trusted},
    ...
    def test_candidate_validation_rejects_stale_git_sha(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            app=Path(tmp)/"Binario Marketing IA.app"; resources=app/"Contents/Resources"; resources.mkdir(parents=True)
            (resources/"PHYSICAL_UAT_CANDIDATE.json").write_text(json.dumps(self._manifest(module,"b"*40,trusted=True)),encoding="utf-8")
    ...
    def test_candidate_validation_accepts_exact_physical_and_validation_roles(self):
        ...
        for trusted in (True,False):
            with self.subTest(trusted=trusted), tempfile.TemporaryDirectory() as tmp:
                app=Path(tmp)/"Binario Marketing IA.app"; resources=app/"Contents/Resources"; resources.mkdir(parents=True)
                path=resources/"PHYSICAL_UAT_CANDIDATE.json"; path.write_text(json.dumps(self._manifest(module,expected,trusted=trusted)),encoding="utf-8")
                manifest_path,actual,actual_trusted=module._validate_candidate(app,expected)
