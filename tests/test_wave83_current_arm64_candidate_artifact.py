...
class Wave83CurrentArm64CandidateArtifactTests(unittest.TestCase):
    ...
    def test_external_delivery_keeps_wave83_exact_identity_with_later_guard(self):
        ...
        self.assertIn('DELIVERY_SCHEMA="binario.marketing.full-mac-delivery.v3"', compact)
        self.assertIn("EXPECTED_RUNTIME_WAVE=76", compact)
        self.assertIn("EXPECTED_GUARD_WAVE=84", compact)
    ...
    def _manifest(module, git_sha: str, *, trusted: bool) -> dict:
        return {
            ...
            "build_origin": {"event": "push" if trusted else "pull_request", "ref": "refs/heads/main" if trusted else "refs/pull/96/merge", "trusted_for_physical_uat": trusted},
