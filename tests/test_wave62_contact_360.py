...
class Wave62Contact360Tests(unittest.TestCase):
    ...
    def _build_evidence_contact(self):
        ...
        contact = self.runtime.create_contact(company_id, {
            "name": "Cliente 360", "organization": "Finca Norte", "email": "cliente@example.com",
            "whatsapp": "+57 300 111 2233", "source": "Lead Intake",
        })
        campaign = self.runtime.create_campaign(company_id, {
            "name": "Campaña comercial", "objective": "LEADS", "status": "IN_PROGRESS",
            "channels": ["email", "whatsapp"], "audience_contact_ids": [contact["id"]],
        })
        link = self.runtime.attribution.create_link(company_id, {
            "campaign_id": campaign["id"], "destination_url": "https://example.com/landing",
            "utm_source": "instagram", "utm_medium": "social", "utm_campaign": "campana-comercial",
            "utm_id": "cmp-360", "utm_content": "reel-a", "utm_source_platform": "meta",
        })
        lead = self.runtime.lead_intake.create(company_id, {
            "connector": "FIRST_PARTY_FORM", "source_ref": "form:contact-360:001", "name": "Cliente 360",
            "email": "cliente@example.com", "source": "Landing comercial", "tracking_link_id": link.id,
            "tracking_code": link.tracking_code, "utm_source": link.utm_source, "utm_medium": link.utm_medium,
            "utm_campaign": link.utm_campaign, "utm_id": link.utm_id, "utm_content": link.utm_content,
            "utm_source_platform": link.utm_source_platform,
        })
        self.runtime.lead_intake.mark_contact_conversion(company_id, lead.id, contact["id"], basis="CREATED_NEW_CONTACT")
        opportunity = self.runtime.create_opportunity(company_id, {
            "contact_id": contact["id"], "title": "Implementación 360", "stage": "INTERESTED",
            "value": 3500000, "currency": "COP",
        })
        self.runtime.attribution.create_claim(company_id, {
            "tracking_code": link.tracking_code, "contact_id": contact["id"], "opportunity_id": opportunity["id"],
    ...
    def test_contact_360_composes_crm_lead_attribution_and_campaign_without_provider_read(self):
        ...
        self.runtime.create_activity(self.company["id"], {
            "contact_id": contact["id"], "opportunity_id": opportunity["id"], "kind": "TASK",
            "summary": "Enviar alcance", "due_at": "2030-01-01T12:00:00+00:00",
        })
    ...
    def test_contact_360_next_action_prioritizes_overdue_followup(self):
        ...
        activity = self.runtime.create_activity(self.company["id"], {
            "contact_id": contact["id"], "opportunity_id": opportunity["id"], "kind": "TASK",
            "summary": "Seguimiento vencido", "due_at": "2020-01-01T12:00:00+00:00",
        })
        payload = self.runtime.contact_360(self.company["id"], contact["id"])
    ...
    def test_contact_360_without_pending_activity_recommends_followup_then_opportunity(self):
        ...
        opportunity = self.runtime.create_opportunity(company_id, {"contact_id": contact["id"], "title": "Venta", "stage": "NEW", "currency": "COP"})
        ...
        lead = self.runtime.lead_intake.create(company_id, {"connector": "MANUAL", "name": "Pipeline", "email": "pipe@example.com", "source": "Feria"})
    ...
    def test_frontend_is_local_read_only_composition_without_polling(self):
        ...
        for marker in ("CRM → EVIDENCIA → SIGUIENTE ACCIÓN", "Origen & atribución", "Atribución verificada", "Campañas relacionadas", "Línea de tiempo", "Abrir Mesa comercial", "/contacts/${encodeURIComponent(id)}/360"):
            self.assertIn(marker, ui)
        self.assertNotIn("setInterval", ui); self.assertNotIn("fetch('https://", ui); self.assertNotIn("sendBeacon", ui)
        self.assertNotIn("method:'POST'", ui); self.assertNotIn("method:'PATCH'", ui)
    ...
    def test_loader_builder_service_workflows_and_release_boundary(self):
        ...
        for audit in ("audit_wave55_lead_intake.sh", "audit_wave59_local_product_integration.sh", "audit_wave60_daily_workdesk.sh", "audit_wave61_commercial_desk.sh", "audit_wave62_contact_360.sh"):
