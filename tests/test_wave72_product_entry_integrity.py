...
class Wave72ProductEntryIntegrityTests(unittest.TestCase):
    ...
    def test_root_injects_deterministic_product_entry(self):
        ...
        self.assertIn("+ Empresa", js); self.assertIn("marketing-company-change", js); self.assertIn("marketing-ops-refreshed", js); self.assertIn("wave72BroadcastContext", js); self.assertIn("/api/product-integrity", js); self.assertNotIn("setInterval", js)
        ...
        for name in REQUIRED_WEB_ASSETS:
            with self.subTest(name=name):
                with urlopen(self.base + "/" + name, timeout=10) as response:
                    self.assertEqual(response.status, 200); self.assertGreater(len(response.read()), 0)

    def test_company_create_select_prerequisites_and_all_safe_projections(self):
        status, before = self._json("/api/product-integrity")
        self.assertEqual(status, 200); self.assertTrue(before["ready"]); self.assertEqual(before["inventory"]["present_web_assets"], 46); self.assertEqual(before["inventory"]["implemented_runtime_methods"], 33); self.assertGreaterEqual(before["inventory"]["registered_apps"], 12)
        status, company = self._post("/api/companies", {"name": "Greenatics UAT"}); self.assertEqual(status, 201)
        status, companies = self._json("/api/companies"); self.assertEqual([row["id"] for row in companies], [company["id"]])
        status, integrity = self._json(f"/api/product-integrity?company_id={company['id']}")
        self.assertEqual(status, 200); self.assertTrue(integrity["ready"], integrity["missing"]); self.assertEqual(integrity["company"]["id"], company["id"]); self.assertEqual(integrity["inventory"]["company_projection_checks"], 27); self.assertEqual(integrity["inventory"]["company_projection_pass"], 27)
        self.assertEqual(integrity["missing"], {"web_assets": [], "runtime_methods": [], "failed_company_projections": []}); self.assertTrue(integrity["safety"]["read_only"]); self.assertFalse(integrity["safety"]["provider_mutation_performed"])
    ...
    def test_release_contract_and_workflow_count_stay_fail_closed(self):
        ...
        service = (ROOT / "src/binario_marketing/service_wave72_app.py").read_text(encoding="utf-8"); self.assertNotIn("RELEASE_READY = True", service)
