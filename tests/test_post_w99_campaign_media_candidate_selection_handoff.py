...
class PostW99CampaignMediaCandidateSelectionHandoffTests(unittest.TestCase):
    ...
    def test_terminal_inherits_planned_only_then_cardinality_hardening(self):
        self.assertTrue(issubclass(AppRuntime, planned_only_base.AppRuntime))
        self.assertTrue(issubclass(planned_only_base.AppRuntime, cardinality_base.AppRuntime))

    def test_planned_only_parent_contract_survives_reconciled_terminal(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime = AppRuntime.create(ROOT, Path(tmp) / "data")
            company = runtime.create_company({"name": "Reconciled Empty Co"})
            try:
    ...
    def test_http_bootstrap_is_after_planned_only_and_parent_chain_survives(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime, company, campaign = self._runtime_with_two_drafts(tmp)
