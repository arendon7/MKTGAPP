...
from binario_marketing import service_post_w99_planned_only_actionability_app as planned_only_base
from binario_marketing import service_post_w99_setup_shadow_action_deduplication_app as setup_shadow_base
...
class PostW99CampaignMediaCandidateSelectionHandoffTests(unittest.TestCase):
    ...
    def test_terminal_inherits_setup_shadow_planned_only_and_cardinality(self):
        self.assertTrue(issubclass(AppRuntime, setup_shadow_base.AppRuntime))
        self.assertTrue(issubclass(setup_shadow_base.AppRuntime, planned_only_base.AppRuntime))
        self.assertTrue(issubclass(planned_only_base.AppRuntime, cardinality_base.AppRuntime))

    def test_parent_action_center_contracts_survive_final_terminal(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime = AppRuntime.create(ROOT, Path(tmp) / "data")
            company = runtime.create_company({"name": "Final Composition Co"})
            try:
    ...
    def test_backend_truth_remains_ambiguous_media_after_setup_shadow_transform(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                action_center = runtime.action_center(company["id"])
                rows = [row for row in action_center["queue"] if row.get("kind") == "finish_creative"]
    ...
    def test_media_selector_remains_separate_from_historical_publication_paid_selector(self):
    ...
    def test_browser_requires_human_click_and_ephemeral_navigation_resolution(self):
    ...
    def test_selection_copy_preserves_backend_ambiguity_as_source_evidence(self):
        source = (ROOT / "web" / "campaign-media-candidate-selection-handoff.js").read_text(encoding="utf-8")
    ...
    def test_http_bootstrap_preserves_cardinality_planned_only_and_media_tail(self):
        with tempfile.TemporaryDirectory() as tmp:
            runtime, company, campaign = self._runtime_with_two_drafts(tmp)
            ...
            try:
                cardinality = urlopen(root + "/campaign-execution-owner-cardinality-hardening.js", timeout=5).read().decode("utf-8")
                planned = urlopen(root + "/planned-only-actionability.js", timeout=5).read().decode("utf-8")
                ...
                self.assertIn("/planned-only-actionability.js", cardinality)
                self.assertIn("data-post-w99-planned-only-actionability", cardinality)
                self.assertIn("/campaign-media-candidate-selection-handoff.js", planned)
                self.assertIn("data-post-w99-campaign-media-candidate-selection-handoff", planned)
    ...
    def test_service_docs_and_release_boundary_are_final_composition_only(self):
