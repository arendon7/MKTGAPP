...
class PortfolioInboxRuntimeTests(unittest.TestCase):
    ...
    def test_browser_defaults_to_portfolio_and_preserves_selective_refresh_authority(self):
        source = (ROOT / "web" / "portfolio-inbox.js").read_text(encoding="utf-8")
        refresh = (ROOT / "web" / "portfolio-inbox-refresh.js").read_text(encoding="utf-8")
        for required in (
            "mode:'PORTFOLIO'", "INBOX / MULTIEMPRESA", "Actualizar Inbox pendiente",
        ...
        self.assertIn("refresh_company_ids", refresh)
        self.assertIn("refresh_overflow", refresh)
        self.assertIn("globalThis.postW99PortfolioInboxRefreshRun=run", refresh)
        self.assertEqual(refresh.count("method:'POST'"), 1)
        self.assertIn("window.confirm", refresh)
        for forbidden in ("setInterval(", "MutationObserver", "sendBeacon", "graph.facebook", "fetch('https://", 'fetch("https://'):
