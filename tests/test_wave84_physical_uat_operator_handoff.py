...
class Wave84PhysicalUATOperatorHandoffTests(unittest.TestCase):
    def test_operator_commands_are_shell_valid_and_never_auto_pass_uat(self):
        ...
        self.assertIn("source_contract_wave", start)

    def test_packager_and_verifier_are_origin_and_source_contract_aware(self):
        ...
        self.assertIn("SOURCE_CONTRACT_WAVE = 94", package)
        ...
        self.assertIn("SOURCE_CONTRACT_WAVE = 94", verify)
        self.assertIn("physical UAT requires a trusted push build", verify)
        self.assertIn("refs/heads/main", verify)
        self.assertNotIn("refs/tags/v", verify)

    def _fixture(self, module, tmp: Path, git_sha: str, *, trusted: bool) -> tuple[Path, Path]:
        delivery_dir = tmp / "delivery"; delivery_dir.mkdir()
        app = tmp / "Binario Marketing IA.app"; resources = app / "Contents/Resources"; resources.mkdir(parents=True)
        ...
        internal = resources / "PHYSICAL_UAT_CANDIDATE.json"; external = delivery_dir / "PHYSICAL_UAT_CANDIDATE.json"
        payload = json.dumps(candidate, ensure_ascii=False, indent=2); internal.write_text(payload, encoding="utf-8"); external.write_text(payload, encoding="utf-8")
        ...
        artifact_name = f"Binario-Marketing-IA-PHYSICAL-UAT-arm64-{git_sha[:12]}.zip"; artifact = delivery_dir / artifact_name; artifact.write_bytes(b"exact-candidate-zip")
        artifact_sha = _sha256(module, artifact); (delivery_dir / f"{artifact_name}.sha256").write_text(f"{artifact_sha}  {artifact_name}\n", encoding="utf-8")
        ...
        delivery = {
            "schema": module.DELIVERY_SCHEMA, "role": role, "build_origin": origin, "physical_uat_eligible": trusted,
            "git_sha": git_sha, "architecture": "arm64", "runtime_wave": 76, "certification_guard_wave": 84, "operator_handoff_wave": 84,
            "source_contract_wave": 94, "source_release_state": "LOCKED_SOURCE", "source_release_tag": None,
            "candidate_source_sha256": "a" * 64, "candidate_manifest_sha256": _sha256(module, internal),
            "artifact": artifact_name, "artifact_sha256": artifact_sha,
            ...
            "physical_uat_required": True, "physical_product_uat_required": True, "release_operational_uat_required": True,
            "automatic_uat_pass": False, "release_ready": False, "release_tag": None,
