...
class Wave84PhysicalUATOperatorHandoffTests(unittest.TestCase):
    ...
    def test_packager_and_verifier_are_origin_aware(self):
        ...
        self.assertIn("SOURCE_CONTRACT_WAVE = 95", package)
        ...
        self.assertIn("SOURCE_CONTRACT_WAVE = 95", verify)
        self.assertIn("_source_digest(source)", verify)
    ...
    def _fixture(self, module, tmp: Path, git_sha: str, *, trusted: bool) -> tuple[Path, Path]:
        ...
        (source / "web/ui.js").write_text("console.log('uat')\n", encoding="utf-8")
    ...
    def test_integrity_verification_accepts_validation_and_physical_roles(self):
        ...
        for trusted in (False, True):
            with self.subTest(trusted=trusted), tempfile.TemporaryDirectory() as tmpdir:
                ...
                self.assertEqual(report["actual_candidate_source_sha256"], report["candidate_source_sha256"])

    def test_extracted_source_drift_invalidates_handoff(self):
        module = _load_verifier()
        with tempfile.TemporaryDirectory() as tmpdir:
            delivery_dir, app = self._fixture(module, Path(tmpdir), "e" * 40, trusted=True)
            (app / "Contents/Resources/source/web/ui.js").write_text("tampered\n", encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "source digest does not match extracted app"):
                module.verify(delivery_dir, app)
