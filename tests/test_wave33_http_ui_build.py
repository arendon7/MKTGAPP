...
class Wave33UiAndBuildContractTests(unittest.TestCase):
    ...
    def test_mac_builder_uses_main_app_service_bridge_and_native_agent(self):
        build = (ROOT / "scripts" / "build_full_mac_app.sh").read_text(encoding="utf-8")
        native_build = (ROOT / "scripts" / "build_native_main_launcher.sh").read_text(encoding="utf-8")
        ...
        bridge = (ROOT / "native" / "background_service_bridge.m").read_text(encoding="utf-8")
        manager = (ROOT / "src" / "binario_marketing" / "background_service.py").read_text(encoding="utf-8")
        ...
        self.assertIn("background_service_bridge.m", native_build)
        self.assertIn("-framework ServiceManagement", native_build)
        self.assertIn("background_agent_launcher.c", background)
        self.assertNotIn("background_service_helper.swift", background)
        ...
        self.assertIn("agentServiceWithPlistName", bridge)
        self.assertIn("registerAndReturnError", bridge)
        self.assertIn("unregisterAndReturnError", bridge)
        self.assertIn("--background-service", launcher)
        self.assertIn("BINARIO_BACKGROUND_SERVICE_HELPER", launcher)
        self.assertIn('[str(self.helper), "--background-service", command]', manager)
        self.assertIn('INSTALLED_APP="/Applications/', audit)
        self.assertIn('"$INSTALLED_LAUNCHER" --background-service status', audit)
        self.assertIn("INSTALLED APP LAUNCHSERVICES PASS", audit)
