...
function socialEscapeText(value){return String(value??'')}
    ...
    <div class="grid-two">
      <div class="composer-card">
        ...
        <form id="social-publication-form" class="stack composer-form">
          ...
          <p id="social-media-note" class="microcopy">Para medios, Meta debe poder descargar el archivo desde Internet. El puente automático desde renders locales se incorporará en el siguiente gate.</p>
...
function setSocialKinds(){
  ...
  const rows=channel.value==='instagram'?[['image','Imagen'],['reel','Reel']]:[['text','Texto'],['link','Enlace'],['image','Imagen']];
  rows.forEach(([value,label])=>kind.append(socialOption(value,label)));
  if(rows.some(([value])=>value===previous))kind.value=previous;
  renderSocialFieldVisibility();
  fillSocialTargets();
}
function renderSocialFieldVisibility(){
  const channel=$('#social-channel')?.value,kind=$('#social-kind')?.value;
  if(!channel||!kind)return;
  $('#social-link-wrap').classList.toggle('hidden',kind!=='link');
  const needsMedia=kind==='image'||kind==='reel'||kind==='video';
  $('#social-media-wrap').classList.toggle('hidden',!needsMedia);
  $('#social-media-note').classList.toggle('hidden',!needsMedia);
}
function fillSocialTargets(){
  const select=$('#social-target');if(!select)return;const previous=select.value;select.replaceChildren();
  const channel=$('#social-channel')?.value||'facebook_page';
  const rows=[];
  for(const page of socialState.pages){
    if(channel==='facebook_page')rows.push({id:page.id,name:page.name||page.id});
    if(channel==='instagram'&&page.instagram)rows.push({id:page.instagram.id,name:`@${page.instagram.username||page.instagram.id} · ${page.name||'Meta'}`});
  }
...
function renderMetaStatus(){
  const status=socialState.status;if(!status)return;const badge=$('#meta-status-badge'),copy=$('#meta-connection-copy'),version=$('#meta-api-version'),scheduler=$('#social-scheduler-chip');
  version.textContent=status.graph_version||'—';
  if(status.configured){badge.textContent='Meta · configurado';badge.classList.add('ok');copy.textContent=`Credencial disponible fuera del proyecto · ${socialState.pages.length} Página(s) · ${socialState.adAccounts.length} cuenta(s) Ads.`}
  else{badge.textContent='Meta · pendiente';badge.classList.remove('ok');copy.textContent=`Falta ${status.missing?.join(', ')||'credencial Meta'}. No se publicará ni se creará pauta hasta configurarla.`}
  scheduler.textContent=status.scheduler?.running?'AUTO ON':'AUTO OFF';
  fillSocialTargets();fillAdAccounts();renderMetaAssets();
}
...
async function refreshMetaConnection(){
  ...
  try{
    socialState.status=await api('/api/meta/status');socialState.pages=[];socialState.adAccounts=[];
    if(socialState.status.configured){
      const results=await Promise.allSettled([api('/api/meta/pages'),api('/api/meta/ad-accounts')]);
      if(results[0].status==='fulfilled')socialState.pages=results[0].value;else toast(`Meta Pages: ${results[0].reason.message}`);
      if(results[1].status==='fulfilled')socialState.adAccounts=results[1].value;else toast(`Meta Ads: ${results[1].reason.message}`);
    }
    renderMetaStatus();
  }catch(err){toast(err.message)}finally{socialState.busy=false;if(button)button.disabled=false}
}
...
function renderSocialPublications(){
  ...
  for(const row of rows){
    const item=el('div','result-item');const when=row.scheduled_for?fmtTime(row.scheduled_for):'sin programación';
    item.append(el('strong','',`${row.status} · ${row.channel==='instagram'?'Instagram':'Facebook'} · ${row.kind}`),el('p','',row.message||'(sin copy)'),el('span','',`${row.target_name||row.target_id} · ${when} · intento ${row.attempts||0}`));
    if(row.remote_id)item.append(el('p','muted',`Meta ID: ${row.remote_id}`));if(row.error)item.append(el('p','muted',row.error));
    const actions=el('div','toolbar');
    if(row.status==='DRAFT'||row.status==='FAILED'){
      const publish=el('button','primary','Publicar ahora');publish.type='button';publish.addEventListener('click',()=>socialPublishNow(row.id));actions.append(publish);
      const queue=el('button','','Encolar ahora');queue.type='button';queue.addEventListener('click',()=>socialQueue(row.id));actions.append(queue);
    }
    if(['DRAFT','QUEUED','FAILED'].includes(row.status)){const cancel=el('button','','Cancelar');cancel.type='button';cancel.addEventListener('click',()=>socialCancel(row.id));actions.append(cancel)}
    item.append(actions);root.append(item)
  }
  if(!rows.length)root.append(el('p','muted','Todavía no hay publicaciones en este proyecto.'));
}
async function refreshSocialProject(){
  const projectId=socialProjectId();if(!projectId)return;
  try{const rows=await api(`/api/projects/${projectId}/publications`);if(state.current?.project?.id!==projectId)return;state.current.publications=rows;renderSocialPublications()}catch(err){toast(err.message)}
}
async function createSocialPublication({publishNow=false}={}){
  const projectId=socialProjectId();if(!projectId)return;const target=$('#social-target'),channel=$('#social-channel'),kind=$('#social-kind');
  if(!target.value){toast('Selecciona una cuenta Meta');return}
  const scheduled=socialJsonDate($('#social-scheduled-for').value);const selected=target.options[target.selectedIndex];
  const payload={channel:channel.value,target_id:target.value,target_name:selected?.textContent||target.value,kind:kind.value,message:$('#social-message').value.trim(),link_url:$('#social-link-url').value.trim()||null,media_url:$('#social-media-url').value.trim()||null,...(!publishNow&&scheduled?{scheduled_for:scheduled}:{})};
  try{
    ...
    $('#social-message').value='';$('#social-link-url').value='';$('#social-media-url').value='';if(!publishNow)$('#social-scheduled-for').value='';
    toast(publishNow?'Publicación enviada a Meta':scheduled?'Publicación programada':'Borrador guardado');await refreshSocialProject();if(typeof refreshTimeline==='function')refreshTimeline();
...
async function socialPublishNow(id){const projectId=socialProjectId();if(!projectId)return;try{await api(`/api/projects/${projectId}/publications/${id}/publish-now`,{method:'POST',body:{}});toast('Publicación procesada');await refreshSocialProject();if(typeof refreshTimeline==='function')refreshTimeline()}catch(err){toast(err.message)}}
...
async function createMetaCampaign(event){
  event.preventDefault();const account=$('#meta-ad-account').value;if(!account){toast('Selecciona una cuenta publicitaria');return}const category=$('#meta-special-category').value;const button=$('#meta-create-campaign');button.disabled=true;
  try{const row=await api('/api/meta/campaigns',{method:'POST',body:{ad_account_id:account,name:$('#meta-campaign-name').value.trim(),objective:$('#meta-campaign-objective').value,special_ad_categories:category?[category]:[]}});const root=$('#meta-campaign-result');root.replaceChildren();const item=el('div','result-item');item.append(el('strong','',`PAUSED · ${row.name}`),el('p','',`${row.objective} · ${row.id}`));root.prepend(item);$('#meta-campaign-name').value='';toast('Campaña creada en PAUSED; no hay gasto activo');if(typeof refreshTimeline==='function')refreshTimeline()}catch(err){toast(err.message)}finally{button.disabled=false}
}
function bindSocialWorkspace(){
  $('#meta-refresh').addEventListener('click',refreshMetaConnection);
  $('#social-channel').addEventListener('change',setSocialKinds);$('#social-kind').addEventListener('change',renderSocialFieldVisibility);
  $('#social-publication-form').addEventListener('submit',event=>{event.preventDefault();createSocialPublication()});
  $('#social-publish-now').addEventListener('click',()=>createSocialPublication({publishNow:true}));
  $('#meta-campaign-form').addEventListener('submit',createMetaCampaign);
  setSocialKinds();
}
async function renderSocialWorkspace(){
  ensureSocialWorkspace();const projectId=socialProjectId();if(!projectId)return;
  if(socialState.projectId!==projectId){socialState.projectId=projectId;renderSocialPublications();await refreshSocialProject()}
  if(!socialState.status)await refreshMetaConnection();else renderMetaStatus();
}
function socialWatchProject(){
  ensureSocialWorkspace();const title=$('#active-project-name');if(title)new MutationObserver(()=>renderSocialWorkspace()).observe(title,{childList:true,characterData:true,subtree:true});
  clearInterval(socialState.refreshTimer);socialState.refreshTimer=setInterval(()=>{if(socialProjectId())refreshSocialProject()},15000);
