...
function operatorSessionProgressRead(companyId){
  ...
  try{
    ...
    value.events=value.events.filter(event=>event&&['ACTION_OPENED','RETURN_OBSERVED'].includes(event.type)).slice(-POST_W99_OPERATOR_SESSION_PROGRESS_MAX_EVENTS);
    return value
...
function operatorSessionProgressWrite(value){
  ...
  value.events=(value.events||[]).slice(-POST_W99_OPERATOR_SESSION_PROGRESS_MAX_EVENTS);
...
function operatorSessionProgressAppend(companyId,event){const value=operatorSessionProgressEnsure(companyId);if(!value)return null;value.events.push(event);return operatorSessionProgressWrite(value)}
