...
function campaignResultsOwnerResolve(row,targetInfo){
  ...
  if(kind==='record_decision'){
    const campaignId=String(context.target_id||''),form=document.querySelector('.w52-decision-form');if(String(postW99CampaignResultsOwnerState.preparedDecisionId||'')===campaignId&&form?.dataset.postW99PreparedCampaignId===campaignId){return controlHandoffSingle([form],campaignResultsOwnerMeta('W52_SUBMIT_CAMPAIGN_DECISION','Completar razonamiento + registrar decisión','CONTROL_GROUP',kind,'El formulario W52 quedó preparado por click humano para la campaña exacta. El usuario conserva decisión, razonamiento y submit final.'))}
