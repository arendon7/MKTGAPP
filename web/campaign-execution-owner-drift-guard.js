...
if(typeof ownerDriftBaseResolve==='function')globalThis.controlHandoffResolve=function(){
  ...
  return{
    schema:baseResult.schema||'binario.marketing.contextual-control-handoff.v1',
    state:'OWNER_STATE_DRIFT',execution,row:action.row,target:null,control:null,owner_drift:drift,
    explanation:`${ownerDriftText(drift.reason)||'El objeto esperado ya no está presente en el estado canónico.'} Revisa el módulo propietario y vuelve a Hoy; no se seleccionó otro registro.`
  }
};
