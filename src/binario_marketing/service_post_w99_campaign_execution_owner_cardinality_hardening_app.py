...
from . import service_post_w99_campaign_execution_owner_relay_app as base
...
def _semantic_media_candidates(code: str, linked_creatives: list[dict]) -> list[dict]:
    normalized = base._text(code).upper()
    if normalized == "FINISH_CREATIVE":
        return [
            row
            for row in linked_creatives
            if base._text(row.get("effective_stage")).upper()
    ...
    if normalized == "PREPARE_DISTRIBUTION":
        return [
            row
            for row in linked_creatives
            if base._text(row.get("effective_stage")).upper() in _READY_CREATIVE_STAGES
        ]
    return []


def _harden_media_resolution(
    *,
    next_action: dict,
    linked_creatives: list[dict],
    inherited_resolution: dict,
) -> dict:
    """Refuse to promote a list-position-derived W64 media_id into identity authority."""
    code = base._text(next_action.get("code")).upper()
    ...
    candidates = [base._candidate_media(row) for row in eligible]
    media_id = base._text(next_action.get("media_id"))
    ...
    candidate_id = base._text((eligible[0].get("media") or {}).get("id"))
...
class AppRuntime(base.AppRuntime):
    """Preserve PR #132 while hardening media cardinality and identity semantics."""
    ...
    def campaign_execution_owner_context(self, company_id: str, campaign_id: str) -> dict:
        ...
        contracts.update({
            "media_identity_requires_semantic_cardinality": True,
            "w64_positional_media_id_is_not_identity_authority": True,
            "single_semantic_media_must_match_w64_media_id": True,
        })
        payload["contracts"] = contracts
        return payload
...
class MarketingHandler(base.MarketingHandler):
    """Append canonical-submit hardening after the certified campaign execution relay."""
