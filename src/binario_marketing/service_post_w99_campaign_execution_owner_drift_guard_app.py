...
def _owner_drift(row: dict) -> dict | None:
    """Return additive drift evidence only for a well-formed, known NO_TARGET shape."""
    ...
    if rule is None:
        return None
    if _text(resolution.get("owner_view")) != rule["owner_view"]:
        return None
    if _text(resolution.get("target_id")):
        return None
    if not _zero_candidate_count(resolution.get("candidate_count")):
        return None
...
def annotate_campaign_execution_owner_drift(payload: dict) -> dict:
    """Annotate provable NO_TARGET drift without changing queue authority or navigation."""
...
class AppRuntime(base.AppRuntime):
    """Expose canonical-owner drift as evidence while preserving W64 and owner authority."""
...
class MarketingHandler(base.MarketingHandler):
    """Load a zero-transport owner-drift browser guard after MEDIA selection."""
