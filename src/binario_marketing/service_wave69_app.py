...
class AppRuntime(base.AppRuntime):
    ...
    def _build_provenance(self) -> dict:
        build = dict(super()._build_provenance())
        build.setdefault("build_event", None)
        build.setdefault("build_ref", None)
        build.setdefault("physical_uat_candidate", False)
        if build.get("source") != "BUILD_PROVENANCE.json":
            return build
        candidates = [
        ...
        for path in candidates:
            ...
            if not isinstance(payload, dict):
                continue
            build["build_event"] = payload.get("build_event")
            build["build_ref"] = payload.get("build_ref")
            build["physical_uat_candidate"] = payload.get("physical_uat_candidate") is True
            break
        return build
    ...
    def physical_uat_preflight(self, company_id: str) -> dict:
        ...
        paths = {
            ...
            "whisper_cli": runtime / "transcription" / "bin" / "whisper-cli",
            "whisper_manifest": runtime / "transcription" / "RUNTIME.json",
        }
        model_dir = runtime / "transcription" / "models"
        model_present = model_dir.is_dir() and any(path.is_file() for path in model_dir.glob("ggml-*.bin"))
        executable_runtime = all(
            path.is_file() and os.access(path, os.X_OK)
            for key, path in paths.items()
            if key not in {"whisper_manifest"}
        )
        ...
        main_candidate_build = (
            build.get("physical_uat_candidate") is True
            and build.get("build_event") == "push"
            and build.get("build_ref") == "refs/heads/main"
        )
        ...
        def check(check_id: str, label: str, passed: bool, detail: str, *, required: bool = True) -> dict:
            return {
                "id": check_id,
                "label": label,
        ...
        checks = [
            check(
                "physical-machine",
                "Mac físico arm64 elegible",
                bool(machine.get("physical_gate_eligible")),
                f"{machine.get('system')} · {machine.get('machine')} · CI={machine.get('is_ci')}",
            ),
            check(
            ...
            check(
                "main-candidate-build",
                "Candidato físico generado desde main",
                main_candidate_build,
                (
                    "push · refs/heads/main · physical_uat_candidate=true"
                    if main_candidate_build
                    else f"event={build.get('build_event') or 'unknown'} · ref={build.get('build_ref') or 'unknown'} · candidate={bool(build.get('physical_uat_candidate'))}"
                ),
            ),
            check(
                "arm64-build",
                ...
                f"architecture={build.get('architecture') or 'unknown'}",
            ),
            check(
                "version-contract",
                "Versión canónica",
                version_matches,
                f"build={build.get('product_version') or 'unknown'} · expected={__version__}",
            ),
            check(
                "embedded-runtime",
                "Runtime embebido completo",
                embedded_runtime_ready,
                "CPython + FFmpeg/FFprobe + whisper-cli + modelo + manifest" if embedded_runtime_ready else "Falta uno o más componentes embebidos del bundle",
            ),
            check(
                "local-data-root",
                "Datos locales accesibles",
                data_root_ready,
                "Directorio local legible/escribible" if data_root_ready else "Directorio local no accesible",
            ),
            check(
                "release-fail-closed",
                "Release continúa bloqueado",
                fail_closed_release,
                "development · ad_hoc · not notarized · RELEASE_READY=False · no tag",
            ),
            check(
                "loopback-default",
                "Servidor local loopback",
                True,
                "127.0.0.1 por defecto; no se requiere bind público",
            ),
        ]
        blockers = [row["id"] for row in checks if row["required"] and not row["passed"]]
        ready = not blockers
        ...
        if overview.get("active_session") and ready:
            ...
            next_action = {
                "code": "START_PHYSICAL_UAT",
                "label": "Iniciar UAT física guiada",
            }
        else:
            next_action = {
                "code": "RESOLVE_PREFLIGHT",
                "label": "Resolver bloqueos de preflight",
            }
        ...
        return {
            ...
            "physical_uat_complete": bool(overview.get("physical_uat_complete")),
            "active_session_id": (overview.get("active_session") or {}).get("id"),
            "scenario_contract": {
                "required": len(required_scenarios),
                "optional": max(0, len(manual_scenarios) - len(required_scenarios)),
            ...
            "release_boundary": {
                ...
                "physical_preflight_is_release_authority": False,
            },
            "safety": {
                "provider_read_performed": False,
                "provider_mutation_performed": False,
                "marketing_mutation_performed": False,
                "physical_uat_result_recorded": False,
    ...
    def do_GET(self) -> None:
        ...
        try:
            if (
                ...
                and parts[3:] == ["physical-uat", "preflight"]
            ):
                self._json(self.server.runtime.physical_uat_preflight(parts[2]))
                return
