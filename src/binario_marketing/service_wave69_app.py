...
from .release_readiness import LOCKED_SOURCE, PREPARED_RELEASE, source_release_state
...
class AppRuntime(base.AppRuntime):
    """Wave 69/W95 verifies physical-UAT prerequisites without satisfying the release gate."""

    def _wave69_resources_root(self) -> Path:
        candidates = [self.repo_root.parent, self.repo_root]
        for candidate in candidates:
            if (candidate / "runtime").is_dir() or (candidate / "BUILD_PROVENANCE.json").is_file():
                return candidate
    ...
    @staticmethod
    def _candidate_source_contract(candidate: dict) -> tuple[str | None, str | None, bool]:
        ...
        if state == LOCKED_SOURCE:
            return LOCKED_SOURCE, None, bool(ready is False and tag is None and wave in {None, SOURCE_CONTRACT_WAVE} and authority_clear)
        if state == PREPARED_RELEASE:
            valid = bool(wave == SOURCE_CONTRACT_WAVE and ready is True and tag == f"v{version}" and ".dev" not in version.lower() and "rc" not in version.lower() and authority_clear)
    ...
    def _trusted_physical_uat_candidate(candidate: dict, build: dict) -> bool:
        ...
        state, tag, source_contract_valid = AppRuntime._candidate_source_contract(candidate)
        try:
            canonical_state = source_release_state(version=__version__, release_ready=RELEASE_READY, release_tag=RELEASE_TAG)
        except ValueError:
            return False
        return bool(
            candidate.get("schema") == "binario.marketing.physical-uat-candidate.v1"
            and candidate.get("role") == "PHYSICAL_UAT_CANDIDATE_ONLY"
            and candidate.get("source_contract_wave") == SOURCE_CONTRACT_WAVE
    ...
    def physical_uat_preflight(self, company_id: str) -> dict:
        ...
        try:
            source_state = source_release_state(version=__version__, release_ready=RELEASE_READY, release_tag=RELEASE_TAG)
            source_contract_valid = source_state in {LOCKED_SOURCE, PREPARED_RELEASE}
        except ValueError:
            source_state = "INVALID_SOURCE_CONTRACT"; source_contract_valid = False
        candidate_state, candidate_tag, candidate_source_contract_valid = self._candidate_source_contract(candidate)
        non_production_candidate = bool(
            source_contract_valid
            and candidate_source_contract_valid
            and candidate.get("source_contract_wave") == SOURCE_CONTRACT_WAVE
            and candidate_state == source_state
            and candidate_tag == RELEASE_TAG
            and str(build.get("release_channel") or "development") == "development"
            and str(build.get("signing_mode") or "ad_hoc") == "ad_hoc"
            and build.get("notarized") is False
        )
        data_root_ready = self.data_root.is_dir() and os.access(self.data_root, os.R_OK | os.W_OK | os.X_OK)

        def check(check_id, label, passed, detail, required=True):
            return {"id": check_id, "label": label, "status": "PASS" if passed else "BLOCKED", "passed": bool(passed), "required": required, "detail": detail}
        ...
        checks = [
            ...
            check("trusted-build-candidate", "Candidato físico exacto de origen GitHub confiable", trusted_candidate, "push · main · PHYSICAL_UAT_CANDIDATE_ONLY" if trusted_candidate else f"role={candidate.get('role') or 'missing'} · event={origin.get('event') or 'unknown'} · ref={origin.get('ref') or 'unknown'}"),
            ...
            check("source-release-contract", "Contrato de fuente W95 coherente", source_contract_valid and candidate_source_contract_valid and candidate.get("source_contract_wave") == SOURCE_CONTRACT_WAVE and candidate_state == source_state and candidate_tag == RELEASE_TAG, f"wave={candidate.get('source_contract_wave')} · state={source_state} · tag={RELEASE_TAG}"),
            ...
            check("release-authority-blocked", "Candidato físico continúa no productivo", non_production_candidate, f"source={source_state} · development channel · ad_hoc · not notarized · no operational/release/publication authority"),
        ...
        if overview.get("active_session") and ready:
            next_action = {"code": "CONTINUE_SESSION", "label": "Continuar sesión UAT activa"}
        elif ready:
            next_action = {"code": "START_PHYSICAL_UAT", "label": "Iniciar UAT física guiada"}
        else:
            next_action = {"code": "RESOLVE_PREFLIGHT", "label": "Resolver bloqueos de preflight"}
        return {
            "schema": "binario.marketing.physical-uat-preflight.v1", "company": {"id": company.id, "name": company.name}, "machine": machine, "build": build,
            "candidate": {"schema": candidate.get("schema"), "role": candidate.get("role"), "git_sha": candidate.get("git_sha"), "source_contract_wave": candidate.get("source_contract_wave"), "candidate_source_sha256": candidate.get("candidate_source_sha256"), "source_release_state": candidate_state, "source_release_tag": candidate_tag, "build_origin": origin, "trusted_for_physical_uat": trusted_candidate},
            "checks": checks, "blockers": blockers, "ready_to_begin_physical_uat": ready, "physical_uat_complete": bool(overview.get("physical_uat_complete")), "active_session_id": (overview.get("active_session") or {}).get("id"),
            "scenario_contract": {"required": len(required_scenarios), "optional": max(0, len(manual_scenarios) - len(required_scenarios)), "automatic_pass": False, "manual_evidence_required": True},
            "next_action": next_action,
            "release_boundary": {"version": __version__, "source_contract_wave": SOURCE_CONTRACT_WAVE, "source_release_state": source_state, "release_ready": RELEASE_READY, "release_tag": RELEASE_TAG, "operational_authorization": False, "release_authority": False, "publication_authority": False, "production_ready": False, "physical_preflight_is_release_authority": False},
            "safety": {"provider_read_performed": False, "provider_mutation_performed": False, "marketing_mutation_performed": False, "physical_uat_result_recorded": False, "background_polling": False, "cloud_required": False},
        }
    ...
    def _static(self, path: str) -> None:
        if path == "/guided-physical-uat.js":
            ...
            if not target.is_file():
                self._error(HTTPStatus.NOT_FOUND, "not found"); return
            bootstrap = """
...
"""
    ...
    def do_GET(self) -> None:
        ...
        try:
            if len(parts) == 5 and parts[:2] == ["api", "companies"] and parts[3:] == ["physical-uat", "preflight"]:
                self._json(self.server.runtime.physical_uat_preflight(parts[2])); return
        except Exception as exc:
            self._wave67_error(exc); return
        super().do_GET()
