...
class AppRuntime(base.AppRuntime):
    """Wave 69/W92 verifies physical-UAT prerequisites without satisfying the release gate."""
    ...
    def _candidate_source_contract(candidate: dict) -> tuple[str | None, str | None, bool]:
        ...
        if state == LOCKED_SOURCE:
            return LOCKED_SOURCE, None, bool(ready is False and tag is None and authority_clear)
        if state == PREPARED_RELEASE:
            valid = bool(ready is True and tag == f"v{version}" and ".dev" not in version.lower() and "rc" not in version.lower() and authority_clear)
    ...
    def physical_uat_preflight(self, company_id: str) -> dict:
        ...
        # The physical candidate remains an engineering/ad-hoc build even when its source is PREPARED_RELEASE.
        # Source preparation freezes version/tag intent; it never substitutes Developer ID/notarization or authority.
        ...
        checks = [
            ...
            check("source-release-contract", "Contrato de fuente coherente", source_contract_valid and candidate_source_contract_valid and candidate_state == source_state and candidate_tag == RELEASE_TAG, f"state={source_state} · tag={RELEASE_TAG}"),
        ...
        return {
            ...
            "candidate": {"schema": candidate.get("schema"), "role": candidate.get("role"), "git_sha": candidate.get("git_sha"), "candidate_source_sha256": candidate.get("candidate_source_sha256"), "source_release_state": candidate_state, "source_release_tag": candidate_tag, "build_origin": origin, "trusted_for_physical_uat": trusted_candidate},
            ...
            "release_boundary": {"version": __version__, "source_release_state": source_state, "release_ready": RELEASE_READY, "release_tag": RELEASE_TAG, "operational_authorization": False, "release_authority": False, "publication_authority": False, "production_ready": False, "physical_preflight_is_release_authority": False},
