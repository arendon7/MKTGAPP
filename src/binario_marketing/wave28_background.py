...
class Wave28SocialScheduler(Wave27SocialScheduler):
    """Wave 27 provider behavior plus cross-process queue ownership."""
    ...
    def start(self) -> None:
        # Base SocialScheduler.start() recovers PUBLISHING rows before its thread
        # starts. With a LaunchAgent another process can legitimately be publishing,
        # so Wave 28 performs recovery only after owning the process lock.
        with self._lock:
...
def install_wave28_scheduler(runtime) -> None:
    previous = runtime.social_scheduler
    if previous is not None:
        previous.shutdown()
    runtime.social_scheduler = Wave28SocialScheduler(runtime.social, on_results=runtime._record_social_results)
    runtime.social_scheduler.start()


__all__ = ["Wave28SocialScheduler", "install_wave28_scheduler"]
