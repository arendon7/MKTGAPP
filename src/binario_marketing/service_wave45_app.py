...
class AppRuntime(base.AppRuntime):
    """Wave 45 adds local, company-scoped CRM CSV portability on top of Wave 44 daily actions."""

    def preview_crm_contacts_csv(self, company_id: str, csv_text: str) -> dict:
        company = self.companies.get(company_id)
        return {"company_id": company.id, **preview_contacts_csv(self.crm, company.id, csv_text)}
    ...
    def export_crm_contacts_csv(self, company_id: str) -> tuple[str, str]:
        ...
        return export_contacts_csv(self.crm, company.id), f"contactos-{safe}.csv"
...
class MarketingHandler(base.MarketingHandler):
    def _wave45_error(self, exc: Exception) -> None:
    ...
    def do_GET(self) -> None:
        ...
        if len(parts) == 6 and parts[:2] == ["api", "companies"] and parts[3:] == ["crm", "contacts", "export"]:
            try:
                ...
                self._headers(HTTPStatus.OK, "text/csv; charset=utf-8", len(body), {"Content-Disposition": f'attachment; filename="{filename}"'})
                self.wfile.write(body)
            except Exception as exc:
                self._wave45_error(exc)
            return
        super().do_GET()
