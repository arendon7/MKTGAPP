...
Desde la reconciliación posterior al PR #140, esta capa se compone **después de Planned-Only Actionability Preservation**. Así se conserva simultáneamente que `PLANNED_ONLY` sea observacional/no ejecutable y que las ambigüedades MEDIA sigan requiriendo selección humana.
...
- Planned-Only Actionability Preservation conserva `CAMPAIGN/PLANNED_ONLY` fuera de Action Center `queue` y de Today sin inventar provider de ejecución.
...
La selección MEDIA es un handoff posterior porque su ambigüedad nace del hardening semántico integrado después de aquel selector histórico.
...
`Campaign MEDIA Candidate Selection Handoff → Planned-Only Actionability Preservation → Campaign Execution Owner Cardinality Hardening → Campaign Coordinate Recovery Guidance → Campaign Coordinate State Decomposition → Campaign Creative Creation Intent Handoff → Campaign Execution Candidate Selector → Campaign Execution Owner Relay → … → Today`
...
La composición browser se implementa sin reemplazar el loader de Planned-Only: el servicio MEDIA hereda `service_post_w99_planned_only_actionability_app` y añade su loader al servir `/planned-only-actionability.js`. De este modo, Cardinality carga Planned-Only y Planned-Only carga MEDIA Selection, preservando la cadena acumulativa.

## Invariants combinados

La reconciliación debe demostrar simultáneamente:

- una fila `CAMPAIGN/PLANNED_ONLY` termina en `observations`, con `NON_ACTIONABLE` y `today_eligible=false`;
- esa fila no ocupa `queue`, `next_action` ni el plan Today;
- una acción `FINISH_CREATIVE` o `PREPARE_DISTRIBUTION` con varios MEDIA elegibles permanece en backend como `AMBIGUOUS_TARGET`;
- el browser no elige candidato hasta `HUMAN_CLICK`;
- la elección no persiste y no ejecuta submit, publicación ni pauta;
- W49/W35 siguen siendo los únicos owners de las mutaciones reales.
