...
La capa no crea una segunda lógica de ejecución. No introduce botones de negocio, endpoints de mutación, reglas de prioridad ni estados de completitud. Observa el contexto efímero de Today/Action Center, exige que Contextual Deep Linking confirme `FOUND_EXACT` y, solo entonces, describe y resalta un control que el owner ya renderizó.

La secuencia acumulativa de producto queda:

`Portfolio → Executive Cockpit → Today → Execution Return → Contextual Deep Linking → Evidence Observability → Portfolio Cadence → Contextual Action Handoff`

Portfolio Cadence y Evidence Observability se conservan como capas anteriores. Cadence sigue siendo una proyección temporal que nunca reprioriza; Evidence sigue siendo autoridad para cobertura/frescura de evidencia local. Action Handoff no reemplaza ninguna de las dos.

## Source-of-truth boundaries

1. **Action Center** sigue siendo la autoridad de prioridad y motivo.
2. **Portfolio Cadence** puede describir semántica temporal, pero no altera prioridad ni inventa deadlines.
3. **Contextual Deep Linking** sigue siendo la autoridad de identidad/navegación exacta.
4. **El módulo propietario** sigue siendo la autoridad del control y de cualquier mutación explícita.
5. **Execution Return** sigue siendo el mecanismo para volver al Plan de hoy y releer Action Center después de actuar.
6. **Action Handoff** es únicamente una capa de presentación que une esos contratos.
...
Busca en el formulario exacto `Crear oportunidad` o `Programar seguimiento`. El formulario y su POST siguen perteneciendo a Mesa Comercial.
...
`service_post_w99_contextual_action_handoff_app` hereda `service_post_w99_portfolio_cadence_app`.

Portfolio Cadence hereda a su vez `service_post_w99_evidence_observability_integrated_app`, por lo que la ascendencia completa se mantiene acumulativa:

`Contextual Action Handoff → Portfolio Cadence → Evidence Observability → Contextual Deep Linking → Execution Return → Today`.

El handler de Action Handoff intercepta únicamente el asset `portfolio-cadence.js` para agregar después el bootstrap de `contextual-action-handoff.js`. No añade endpoints de negocio. La cadena browser queda:
...
- `portfolio_cadence_never_reprioritizes = true`
