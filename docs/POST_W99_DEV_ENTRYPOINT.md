...
Campaign Coordinate State Decomposition continúa backend-only. Cardinality Hardening se carga inmediatamente después del adapter de Recovery Guidance. Planned-Only Actionability Preservation se carga inmediatamente después de Cardinality Hardening. Setup Shadow Action Deduplication es backend-only y transforma la lectura heredada de Action Center después de Planned-Only, por lo que no necesita asset JS propio. Campaign MEDIA Candidate Selection Handoff hereda ese runtime terminal y añade su loader al servir el último asset browser existente, `/planned-only-actionability.js`; por tanto no bypassa Setup Shadow ni reemplaza la proyección Action Center corregida.
