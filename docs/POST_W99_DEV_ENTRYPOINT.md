...
`serve-dev` resuelve `service_post_w99_dev_app`, que actualmente carga Action Center + Pipeline Priority + Global Navigator + Commercial Outcome Intelligence + Decision Review + Portfolio Control Tower + Executive Marketing Cockpit + Today / Operator Execution + Execution Return Flow + Contextual Deep Linking + Evidence Observability + Portfolio Cadence + Contextual Control Handoff + Opportunity Follow-up Control + Existing Activity Reschedule Control + Campaign Results Owner Handoff + Execution Owner Relay.
...
- **Execution Owner Relay** revalida la acción actual de W64 y resuelve el owner final exacto para publicación, pauta o creativo solo cuando existe una identidad única en los stores canónicos. Nunca usa el primer candidato por posición; si la acción cambió o hay 0/>1 candidatos, falla cerrado. El segundo salto requiere click humano y sigue usando los controles propietarios existentes.

`service_post_w99_execution_owner_relay_app` es ahora el terminal de composición de `serve-dev`. Extiende a `service_post_w99_campaign_results_owner_handoff_app` sin reemplazarlo. La ascendencia explícita de las capas superiores es:

`service_post_w99_execution_owner_relay_app` → `service_post_w99_campaign_results_owner_handoff_app` → `service_post_w99_existing_activity_reschedule_control_app` → `service_post_w99_opportunity_followup_control_app` → `service_post_w99_contextual_control_handoff_app` → `service_post_w99_portfolio_cadence_app` → `service_post_w99_evidence_observability_integrated_app` → `service_post_w99_contextual_deep_linking_app` → `service_post_w99_execution_return_app` → Today.
...
`Today → Execution Return → Contextual Deep Linking → Evidence Observability → Portfolio Cadence → Contextual Control Handoff → Opportunity Follow-up Control → Existing Activity Reschedule Control → Campaign Results Owner Handoff → Execution Owner Relay`
