...
`service_post_w99_campaign_coordinate_actionability_app` es ahora el terminal de composición de `serve-dev`. La ascendencia explícita es:
...
Campaign Coordinate State Decomposition continúa backend-only. Cardinality Hardening se carga inmediatamente después del adapter de Recovery Guidance. Planned-Only Actionability Preservation se carga inmediatamente después de Cardinality Hardening. Setup Shadow Action Deduplication es backend-only y transforma la lectura heredada de Action Center después de Planned-Only, por lo que no necesita asset JS propio. Campaign MEDIA Candidate Selection Handoff hereda ese runtime y añade su loader al servir `/planned-only-actionability.js`. Finalmente, Campaign Coordinate Actionability Preservation hereda el terminal MEDIA y añade `/campaign-coordinate-actionability.js` al servir `/campaign-media-candidate-selection-handoff.js`; así puede corregir la elegibilidad de `COORDINATE` después de que #136/#137 hayan producido el diagnóstico/recovery y sin bypassar Setup Shadow ni Planned-Only.
