...
`service_post_w99_campaign_execution_owner_drift_guard_app` es ahora el terminal de composición de `serve-dev`. `service_post_w99_setup_readiness_owner_handoff_app` permanece como parent inmediato y además se importa explícitamente en el entrypoint únicamente como breadcrumb de compatibilidad para los contratos acumulativos. La ascendencia es:
