...
- **Campaign MEDIA Candidate Selection Handoff** consume exclusivamente `AMBIGUOUS_TARGET + MEDIA` de esos dos códigos y permite elegir un `media_id` mediante click humano. Conserva la resolución backend ambigua como evidencia, crea solo una resolución `navigation_only` efímera y no persiste selección, ranking ni recomendación.
...
Campaign Coordinate State Decomposition continúa backend-only. Cardinality Hardening se carga después de Recovery Guidance; MEDIA Candidate Selection Handoff se carga después de Cardinality Hardening y solo intercepta sus ambigüedades MEDIA válidas. Todo lo demás se delega a la cadena ya certificada.
