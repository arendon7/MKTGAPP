...
- **Setup Shadow Action Deduplication** elimina únicamente agregados SETUP cuya cobertura total por acciones canónicas puede demostrarse; preserva evidencia en `shadowed_actions` y falla cerrado ante cobertura parcial, huérfana o incoherente.

`service_post_w99_setup_shadow_action_deduplication_app` es ahora el terminal de composición de `serve-dev`. La ascendencia explícita es:
