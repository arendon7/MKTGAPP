...
Las capas superiores son acumulativas y conservan sus autoridades: Portfolio/Executive/Today presentan y ordenan trabajo ya canónico; los handoffs posteriores solo refinan navegación, observabilidad o controles cuando identidad y cardinalidad pueden demostrarse.
...
- **Campaign Execution Owner Drift Guard** observa únicamente `NO_TARGET` bien formado para owners/códigos conocidos, conserva intactos W64, prioridad y acción, y convierte el callejón sin target en revisión humana explícita del owner sin seleccionar sustitutos.
...
Campaign Coordinate State Decomposition continúa backend-only. Cardinality Hardening se carga inmediatamente después del adapter de Recovery Guidance. Planned-Only Actionability Preservation se carga inmediatamente después de Cardinality Hardening. Setup Shadow Action Deduplication es backend-only y transforma la lectura heredada de Action Center después de Planned-Only, por lo que no necesita asset JS propio. Campaign MEDIA Candidate Selection Handoff hereda ese runtime y añade su loader sobre `/planned-only-actionability.js`. Campaign Execution Owner Drift Guard hereda MEDIA Selection y añade su loader al servir `/campaign-media-candidate-selection-handoff.js`; por tanto conserva Setup Shadow, Planned-Only y la semántica de selección humana ya certificadas.
