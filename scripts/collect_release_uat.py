...
def render_markdown(report: dict[str, Any]) -> str:
    lines=["# BINARIO Marketing IA · Physical UAT Evidence","",f"- Generated: `{report['generated_at']}`",f"- Git SHA: `{report['git_sha']}`",f"- Candidate source SHA-256: `{report['candidate_source_sha256']}`",f"- Architecture: `{report['architecture']}`",f"- Version: `{report['version']}`",f"- Overall: **{report['overall']}**","","## Automatic checks"]
    for row in report["automatic_checks"]: lines.append(f"- **{'PASS' if row['ok'] else 'FAIL'}** · {row['name']}")
    lines += ["","## Manual gates"]
    for row in report["manual_steps"]: lines.append(f"- **{row['status']}** · `{row['id']}` — {row['step']}")
    lines += ["","## Rule","","Este archivo se genera inicialmente con `uat_passed=false`. UAT solo puede considerarse PASS cuando todos los gates manuales se ejecuten sobre este mismo SHA, digest de fuente y candidato arm64 generado desde un origen GitHub confiable.","La generación del reporte no habilita `RELEASE_READY`, no crea un tag y no sustituye firma Developer ID ni notarización.",""]
    return "\n".join(lines)


def main() -> int:
    ap=argparse.ArgumentParser(description="Collect physical release UAT evidence for one exact BINARIO Marketing Mac candidate."); ap.add_argument("--app",type=Path,required=True); ap.add_argument("--output",type=Path,required=True); args=ap.parse_args()
    ...
    trusted_origin=bool(candidate.get("role")==PHYSICAL_ROLE and origin.get("event")=="push" and (ref=="refs/heads/main" or ref.startswith("refs/tags/v")) and origin.get("trusted_for_physical_uat") is True and candidate.get("physical_uat",{}).get("eligible_build_origin") is True)
    candidate_consistent=bool(candidate.get("schema")==CANDIDATE_SCHEMA and trusted_origin and candidate.get("git_sha")==provenance.get("git_sha") and candidate.get("architecture")==provenance.get("architecture")=="arm64" and candidate.get("product_version")==provenance.get("product_version") and isinstance(candidate.get("candidate_source_sha256"),str) and len(candidate.get("candidate_source_sha256"))==64 and candidate.get("runtime_wave")==76 and candidate.get("certification_guard_wave")==84 and candidate.get("physical_uat",{}).get("automatic_pass") is False)
    ...
    report={"schema":SCHEMA,"generated_at":datetime.now(timezone.utc).isoformat(),"host":{"system":host_system,"release":platform.release(),"machine":host_machine,"is_ci":is_ci},"app":str(app),"git_sha":provenance.get("git_sha"),"architecture":provenance.get("architecture"),"version":provenance.get("product_version"),"candidate_schema":candidate.get("schema"),"candidate_role":candidate.get("role"),"candidate_build_origin":origin,"candidate_source_sha256":candidate.get("candidate_source_sha256"),"candidate_manifest_sha256":_sha256(candidate_path) if candidate_path.is_file() else None,"runtime_wave":candidate.get("runtime_wave"),"signing_mode":provenance.get("signing_mode"),"notarized":provenance.get("notarized"),"automatic_checks":checks,"manual_steps":manual,"automatic_passed":automatic_ok,"uat_passed":False,"overall":"AUTOMATIC_FAIL" if not automatic_ok else "AUTOMATIC_PASS_MANUAL_PENDING"}
    out=args.output.expanduser().resolve(); out.mkdir(parents=True,exist_ok=True)
    (out/"release-uat-evidence.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8"); (out/"release-uat-evidence.md").write_text(render_markdown(report),encoding="utf-8")
    print(json.dumps({"overall":report["overall"],"output":str(out),"git_sha":report["git_sha"],"candidate_source_sha256":report["candidate_source_sha256"],"architecture":report["architecture"]},ensure_ascii=False,indent=2)); return 0 if automatic_ok else 2
