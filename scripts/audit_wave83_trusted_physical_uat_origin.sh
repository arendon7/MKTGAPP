#!/bin/bash
set -euo pipefail
APP="${1:?usage: audit_wave83_trusted_physical_uat_origin.sh <app>}"
...
[[ -x "$PY" && -f "$MANIFEST" && -f "$SERVICE" ]] || { echo 'Wave 83 origin audit files missing' >&2; exit 1; }
...
assert row.get('certification_guard_wave')==83, row
...
print(f"W83 origin PASS: event={event} ref={ref} trusted={expected}")
PY
...
echo 'WAVE 83 TRUSTED PHYSICAL UAT ORIGIN AUDIT PASS'
