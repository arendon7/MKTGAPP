...
On an eligible delivery, expand `{handoff_archive}` and double-click `START_PHYSICAL_UAT.command` on a real Apple Silicon Mac. It verifies checksum, code-sign integrity, exact candidate binding and physical host before initializing evidence or opening the app.

## Phase A · In-app physical product UAT

Execute the five required scenarios: `company-switch`, `inbox-to-crm`, `pipeline-followup`, `campaign-execution`, `results-decision`. `optional-ai` remains optional. Record real observations and finish explicitly.

## Phase B · Release operational UAT

Use `RECORD_RELEASE_UAT.command` once per manual gate. Every result requires PASS/FAIL plus a concrete observation note.

## Boundary

Neither phase changes `RELEASE_READY`, creates tags, signs with Developer ID, notarizes the app, publishes marketing or activates ads. A validation-only PR artifact must never be used as physical evidence.
"""
...
def package(app:Path,out_dir:Path,expected_git_sha:str)->dict[str,Any]:
    ...
    verifier_path=out_dir/"PHYSICAL_UAT_HANDOFF_VERIFY.py"; starter_path=out_dir/"START_PHYSICAL_UAT.command"; recorder_path=out_dir/"RECORD_RELEASE_UAT.command"; guide_path=out_dir/"PHYSICAL_UAT_OPERATOR.md"
    ...
    for target,source in {verifier_path:scripts/"verify_physical_uat_handoff.py",starter_path:scripts/"start_physical_uat.command",recorder_path:scripts/"record_release_uat.command"}.items():
        if not source.is_file(): raise ValueError(f"operator handoff helper missing: {source}")
        shutil.copy2(source,target)
    starter_path.chmod(0o755); recorder_path.chmod(0o755)
    ...
    delivery={"schema":DELIVERY_SCHEMA,"role":manifest.get("role"),"product":manifest.get("product"),"git_sha":expected_git_sha,"architecture":"arm64","product_version":manifest.get("product_version"),"runtime_wave":EXPECTED_RUNTIME_WAVE,"certification_guard_wave":EXPECTED_GUARD_WAVE,"operator_handoff_wave":OPERATOR_HANDOFF_WAVE,"operator_handoff_archive":handoff_name,"build_origin":manifest.get("build_origin"),"candidate_source_sha256":manifest.get("candidate_source_sha256"),"candidate_manifest_sha256":candidate_manifest_sha,"artifact":zip_name,"artifact_sha256":artifact_sha,"handoff_verifier_sha256":_sha256(verifier_path),"start_command_sha256":_sha256(starter_path),"record_command_sha256":_sha256(recorder_path),"operator_guide_sha256":_sha256(guide_path),"physical_uat_eligible":trusted,"physical_uat_required":True,"physical_product_uat_required":True,"release_operational_uat_required":True,"automatic_uat_pass":False,"release_ready":False,"release_tag":None,"production_ready":False}
    ...
    for source in (zip_path,checksum_path,delivery_path,external_manifest_path,external_summary_path,verifier_path,starter_path,recorder_path,guide_path): shutil.copy2(source,handoff_root/source.name)
    (handoff_root/starter_path.name).chmod(0o755); (handoff_root/recorder_path.name).chmod(0o755)
