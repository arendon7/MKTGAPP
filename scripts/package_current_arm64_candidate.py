...
def package(app: Path,out_dir: Path,expected_git_sha: str) -> dict[str,Any]:
    ...
    # Keep Wave 83 external filename contract stable; role/eligibility is authoritative in manifests.
    zip_name=f"Binario-Marketing-IA-PHYSICAL-UAT-arm64-{short_sha}.zip"
    zip_path=out_dir/zip_name; checksum_path=out_dir/f"{zip_name}.sha256"; delivery_path=out_dir/"FULL_MAC_DELIVERY.json"
    external_manifest_path=out_dir/"PHYSICAL_UAT_CANDIDATE.json"; external_summary_path=out_dir/"PHYSICAL_UAT_CANDIDATE.md"
    ...
    delivery={
        ...
        "runtime_wave":EXPECTED_RUNTIME_WAVE,"certification_guard_wave":EXPECTED_GUARD_WAVE,"build_origin":manifest.get("build_origin"),"candidate_source_sha256":manifest.get("candidate_source_sha256"),
        "candidate_manifest_sha256":candidate_manifest_sha,"artifact":zip_name,"artifact_sha256":artifact_sha,"physical_uat_eligible":trusted,"physical_uat_required":True,"automatic_uat_pass":False,
        "release_ready":False,"release_tag":None,"production_ready":False,
    }
    delivery_path.write_text(json.dumps(delivery,ensure_ascii=False,indent=2,sort_keys=True)+"\n",encoding="utf-8"); return delivery


def main() -> int:
    parser=argparse.ArgumentParser(description="Package the exact current arm64 build and emit authoritative role/eligibility metadata.")
    parser.add_argument("--app",type=Path,required=True); parser.add_argument("--out",type=Path,default=Path(".")); parser.add_argument("--git-sha",default=os.environ.get("GITHUB_SHA")); args=parser.parse_args()
