...
def _validate_candidate(app:Path,expected_git_sha:str)->tuple[Path,dict[str,Any],bool]:
    ...
    if boundary.get("release_ready") is not False or boundary.get("release_tag") is not None or boundary.get("production_ready") is not False: raise ValueError("candidate source safety boundary drift")
...
def _operator_guide(*,git_sha:str,artifact:str,artifact_sha:str,handoff_archive:str,source_sha:str,role:str,eligible:bool,version:str)->str:
...
- Explicit release authorization layer: `Wave 89`
...
- Automatic release authority: **NO**
- Automatic UAT pass: **NO**

## Start

The handoff may be built and audited in pull-request CI, but `START_PHYSICAL_UAT.command` only proceeds when this exact delivery is `PHYSICAL_UAT_CANDIDATE_ONLY` and its recorded origin is a controlled GitHub `push` to `refs/heads/main`.

Tag builds are distribution rebuilds and are never exact physical-UAT candidates.
...
## Phase D · Explicit human release authorization

Only for a stable tested version and only after Phase C passes, run `AUTHORIZE_RELEASE.py` against the combined attestation. It requires the exact tag `v<tested-version>`, an identified human approver, a concrete note, and the literal confirmation `AUTHORIZE EXACT TESTED RELEASE`.

The resulting `release-authorization.json` is the only W89 artifact that carries explicit release authority. It remains bound to this exact SHA, tested source digest and combined physical-UAT attestation. It does not sign, notarize or publish anything by itself.

## Boundary

The source tree intentionally keeps `RELEASE_READY=False` and `RELEASE_TAG=None`. A future tag run must receive both the W85 attestation and the W89 authorization through protected Actions secrets, then independently pass W87/W88 Developer ID, notarization, Gatekeeper and distribution-rebuild gates before publication.
"""
...
def package(app:Path,out_dir:Path,expected_git_sha:str)->dict[str,Any]:
    ...
    verifier_path=out_dir/"PHYSICAL_UAT_HANDOFF_VERIFY.py"; starter_path=out_dir/"START_PHYSICAL_UAT.command"; recorder_path=out_dir/"RECORD_RELEASE_UAT.command"; finalizer_path=out_dir/"FINALIZE_PHYSICAL_UAT.py"; finalize_command_path=out_dir/"FINALIZE_PHYSICAL_UAT.command"; authorizer_path=out_dir/"AUTHORIZE_RELEASE.py"; guide_path=out_dir/"PHYSICAL_UAT_OPERATOR.md"
    ...
    helpers={
        ...
        authorizer_path:scripts/"authorize_release.py",
    }
    ...
    guide_path.write_text(_operator_guide(git_sha=expected_git_sha,artifact=zip_name,artifact_sha=artifact_sha,handoff_archive=handoff_name,source_sha=str(manifest.get("candidate_source_sha256") or ""),role=str(manifest.get("role") or ""),eligible=trusted,version=str(manifest.get("product_version") or "")),encoding="utf-8")
    delivery={
        ...
        "runtime_wave":EXPECTED_RUNTIME_WAVE,"certification_guard_wave":EXPECTED_GUARD_WAVE,"operator_handoff_wave":OPERATOR_HANDOFF_WAVE,"combined_attestation_wave":COMBINED_ATTESTATION_WAVE,"release_authorization_wave":RELEASE_AUTHORIZATION_WAVE,
        ...
        "combined_finalizer_sha256":_sha256(finalizer_path),"finalize_command_sha256":_sha256(finalize_command_path),"release_authorizer_sha256":_sha256(authorizer_path),"operator_guide_sha256":_sha256(guide_path),
        "physical_uat_eligible":trusted,"physical_uat_required":True,"physical_product_uat_required":True,"release_operational_uat_required":True,"combined_attestation_required_before_release_transport":True,"explicit_release_authorization_required":True,
    ...
    for source in (zip_path,checksum_path,delivery_path,external_manifest_path,external_summary_path,verifier_path,starter_path,recorder_path,finalizer_path,finalize_command_path,authorizer_path,guide_path): shutil.copy2(source,handoff_root/source.name)
