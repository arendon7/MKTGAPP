...
def _release_contract(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == LOCKED_SOURCE:
        if release_ready or release_tag is not None:
            raise ValueError("locked source release boundary is inconsistent")
    elif mode == PREPARED_RELEASE:
        if not release_ready:
            raise ValueError("prepared release boundary must set release_ready")
        if _development_version(version):
            raise ValueError("prepared release boundary cannot use a development/RC version")
        if release_tag != f"v{version}":
            raise ValueError("prepared release boundary tag/version mismatch")
    else:
        raise ValueError(f"unknown source release contract mode: {mode}")
    if boundary.get("production_ready") is not False:
        raise ValueError("physical candidate cannot claim production readiness")
    if boundary.get("release_authority") not in {None, False}:
        raise ValueError("physical candidate cannot carry release authority")
    if boundary.get("operational_authorization") not in {None, False}:
        raise ValueError("physical candidate cannot carry operational authorization")
    return {
        "mode": mode,
...
def _validate_candidate(app: Path, expected_git_sha: str) -> tuple[Path, dict[str, Any], bool]:
    ...
    release_contract = _release_contract(manifest)
    ...
    manifest["_normalized_release_contract"] = release_contract
...
- Prepared-release contract layer: `Wave 91`
- Candidate source SHA-256: `{source_sha}`
- Source release contract: `{release_contract['mode']}`
- Source RELEASE_READY: `{release_contract['release_ready']}`
- Source RELEASE_TAG: `{release_contract['release_tag']}`
...
A `PREPARED_RELEASE` source contract is allowed here. It means the stable version and canonical future tag were declared **before** UAT so that the exact tested commit can later be tagged without changing source. It does not mean the app is signed for distribution, notarized, operationally authorized, production-ready or published.
...
After Phase A and Phase B are complete, double-click `FINALIZE_PHYSICAL_UAT.command` and select the Phase A JSON. Wave 85 verifies both evidence layers against this exact trusted candidate and writes a sanitized combined attestation under `PHYSICAL_UAT_EVIDENCE/combined/`. Wave 91 binds the source release contract into that attestation.

## Boundary

The combined attestation does not create a tag, sign with Developer ID, notarize the app, publish a GitHub Release, mutate providers, publish marketing or activate ads. Tag builds remain source-equivalent distribution rebuilds and are never exact physical-UAT candidates.
"""
...
def package(app: Path, out_dir: Path, expected_git_sha: str) -> dict[str, Any]:
    ...
    release_contract = manifest.pop("_normalized_release_contract")
    ...
    finalizer_path = out_dir / "FINALIZE_PHYSICAL_UAT.py"
    finalize_command_path = out_dir / "FINALIZE_PHYSICAL_UAT.command"
    ...
    finalize_command_path.chmod(0o755)
    ...
    delivery = {
        ...
        "combined_attestation_wave": COMBINED_ATTESTATION_WAVE,
        ...
        "combined_finalizer_sha256": _sha256(finalizer_path),
        "finalize_command_sha256": _sha256(finalize_command_path),
        ...
        "combined_attestation_required_before_release_transport": True,
        "automatic_uat_pass": False,
        "release_ready": release_contract["release_ready"],
        "release_tag": release_contract["release_tag"],
        "operational_authorization": False,
        "release_authority": False,
        "production_ready": False,
    }
    ...
    for source in (
        zip_path, checksum_path, delivery_path, external_manifest_path, external_summary_path,
        verifier_path, starter_path, recorder_path, finalizer_path, finalize_command_path, guide_path,
    ):
