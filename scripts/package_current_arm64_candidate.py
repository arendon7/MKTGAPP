...
def _source_contract(manifest: dict[str, Any]) -> tuple[str, str | None]:
    boundary = manifest.get("release_boundary") if isinstance(manifest.get("release_boundary"), dict) else {}
    version = str(manifest.get("product_version") or "")
    state = manifest.get("source_release_state") or boundary.get("source_release_state")
    ready = boundary.get("release_ready")
    tag = boundary.get("release_tag")
    if state is None and ready is False and tag is None:
        state = LOCKED_SOURCE
    if boundary.get("operational_authorization") not in {None, False} or boundary.get("release_authority") not in {None, False} or boundary.get("publication_authority") not in {None, False} or boundary.get("production_ready") is not False:
        raise ValueError("candidate unexpectedly carries release authority")
    if state == LOCKED_SOURCE:
        if ready is not False or tag is not None:
            raise ValueError("LOCKED_SOURCE candidate release boundary drift")
    elif state == PREPARED_RELEASE:
        if ready is not True or tag != f"v{version}" or ".dev" in version.lower() or "rc" in version.lower():
            raise ValueError("PREPARED_RELEASE candidate version/tag contract drift")
    else:
...
def _validate_candidate(app: Path, expected_git_sha: str) -> tuple[Path, dict[str, Any], bool, str, str | None]:
    ...
    source_state, source_tag = _source_contract(manifest)
    ...
    return manifest_path, manifest, trusted, source_state, source_tag


def _operator_guide(*, git_sha: str, artifact: str, artifact_sha: str, handoff_archive: str, source_sha: str, role: str, eligible: bool, source_state: str, source_tag: str | None) -> str:
...
`PREPARED_RELEASE` means only that the stable version and future tag were frozen in source before this UAT. It does not mean this ad-hoc physical candidate is a distributable production build.
...
After Phase A and Phase B are complete, double-click `FINALIZE_PHYSICAL_UAT.command` and select the Phase A JSON. Wave 85/W95 verifies both evidence layers against this exact trusted main candidate and preserves the source release state in the sanitized combined attestation.

## Boundary

The combined attestation still does not create tags, sign with Developer ID, notarize the app, publish marketing or activate ads. A future tag workflow may consume it only when it proves `PREPARED_RELEASE` for that exact same SHA/source/tag/version.
"""
...
def package(app: Path, out_dir: Path, expected_git_sha: str) -> dict[str, Any]:
    ...
    manifest_path, manifest, trusted, source_state, source_tag = _validate_candidate(app, expected_git_sha)
    ...
    guide_path.write_text(_operator_guide(git_sha=expected_git_sha, artifact=zip_name, artifact_sha=artifact_sha, handoff_archive=handoff_name, source_sha=str(manifest.get("candidate_source_sha256") or ""), role=str(manifest.get("role") or ""), eligible=trusted, source_state=source_state, source_tag=source_tag), encoding="utf-8")
    delivery = {
        ...
        "runtime_wave": EXPECTED_RUNTIME_WAVE, "certification_guard_wave": EXPECTED_GUARD_WAVE, "operator_handoff_wave": OPERATOR_HANDOFF_WAVE, "combined_attestation_wave": COMBINED_ATTESTATION_WAVE, "source_contract_wave": SOURCE_CONTRACT_WAVE,
        ...
        "release_ready": False, "release_tag": None,
    ...
    for source in (zip_path, checksum_path, delivery_path, external_manifest_path, external_summary_path, verifier_path, starter_path, recorder_path, finalizer_path, finalize_command_path, guide_path):
        shutil.copy2(source, handoff_root / source.name)
    for executable in (starter_path.name, recorder_path.name, finalize_command_path.name):
        (handoff_root / executable).chmod(0o755)
