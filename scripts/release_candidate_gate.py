...
def _combined_source_contract(binding: dict[str, Any]) -> tuple[str | None, str | None, bool]:
    ...
    # W85/W86 legacy attestations predate the explicit source-state field. They are
    # diagnostic-only LOCKED_SOURCE evidence and are never accepted by --production.
    if state is None and tag is None:
        state = LOCKED_SOURCE
    if state == LOCKED_SOURCE:
        return LOCKED_SOURCE, None, tag is None
    if state == PREPARED_RELEASE:
        valid = bool(tag == f"v{version}" and ".dev" not in version.lower() and "rc" not in version.lower())
...
def main() -> int:
    ...
    if provenance and architecture == "arm64" and rebuild is None:
        ...
        if not candidate_consistent:
            _append_blocker(report, "physical_uat_candidate_manifest_missing_or_invalid", "uat", "El bundle arm64 no contiene un manifiesto W84/W92 válido de origen main confiable.")
    ...
    if args.production:
        ...
        if not uat_source_contract_valid or uat_source_state != PREPARED_RELEASE:
            _append_blocker(report, "prepared_release_uat_required", "uat", "Producción exige UAT física ejecutada sobre el mismo commit PREPARED_RELEASE; evidencia LOCKED_SOURCE/legacy no puede autorizar publicación.")
