...
def _load_version(repo: Path) -> tuple[str, bool, str | None, str]:
    ...
    try:
        ...
        return __version__, RELEASE_READY, RELEASE_TAG, source_release_state(version=__version__, release_ready=RELEASE_READY, release_tag=RELEASE_TAG)
    finally:
        try:
...
def audit(repo: Path) -> dict[str, Any]:
    repo = repo.resolve()
    try:
        version, release_ready, release_tag, source_state = _load_version(repo)
        source_contract_error = None
    except ValueError as exc:
        sys.path.insert(0, str(repo / "src"))
        try:
            from binario_marketing.version import RELEASE_READY, RELEASE_TAG, __version__
            version, release_ready, release_tag = __version__, RELEASE_READY, RELEASE_TAG
        finally:
            try:
                sys.path.remove(str(repo / "src"))
            except ValueError:
    ...
    structural = {
        ...
        "prepared_release_uat_preflight": "--expected-source-release-state PREPARED_RELEASE" in workflow and '--expected-release-tag "$GITHUB_REF_NAME"' in workflow,
        "prepared_release_production_gate": all(marker in gate for marker in ("prepared_release_uat_required", "prepared_release_tag_mismatch", "prepared_release_source_required", "PREPARED_RELEASE")),
        ...
        "release_tag_verifier": "verify_release_tag.py" in workflow and "verify_pipeline_contract" in tag_verifier and "PREPARED_RELEASE" in tag_verifier,
    ...
    external_requirements = {
        ...
        "physical_uat_was_executed_on_prepared_release_same_sha": False,
