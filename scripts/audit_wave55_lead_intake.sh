...
GUARD="$RES/source/src/binario_marketing/service_wave55_guard_app.py"
STORE="$RES/source/src/binario_marketing/lead_intake_store.py"
LOADER="$RES/source/web/audiences-wave39-loader.js"
[[ -f "$UI" && -f "$SERVICE" && -f "$GUARD" && -f "$STORE" && -f "$LOADER" ]] || { echo "Wave 55 audit: source missing" >&2; exit 3; }
/usr/bin/grep -q 'service_wave55_guard_app import serve' "$RES/launch.py" || { echo "Wave 55 audit: Mac launch is not using certified Wave 55 guard" >&2; exit 3; }
...
/usr/bin/grep -q '_lead_attribution_prepared.*company.id.*lead' "$GUARD" || { echo "Wave 55 audit: pre-conversion attribution integrity guard missing" >&2; exit 3; }
/usr/bin/grep -q 'before any CRM mutation' "$GUARD" || { echo "Wave 55 audit: preflight ordering contract missing" >&2; exit 3; }
