...
[[ -f "$WORKFLOW" && -f "$BUILDER" && -f "$CURRENT" && -f "$GUARDED" ]]
...
/usr/bin/grep -q 'not yet certified for x86_64' "$BUILDER"
/usr/bin/grep -q 'refusing to fall back to the historical W45 base builder' "$BUILDER"
